...
CBITMAP *cbitmap_new(const UINT32 max_bits)
{
    ...
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBITMAP, &cbitmap, LOC_CBITMAP_0001);
    if(NULL_PTR != cbitmap)
    {
...
EC_BOOL  cbitmap_free(CBITMAP *cbitmap)
{
    if(NULL_PTR != cbitmap)
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBITMAP, cbitmap, LOC_CBITMAP_0004);
    }
    return (EC_TRUE);
}
...
EC_BOOL  cbitmap_dfs_flush(const CBITMAP *cbitmap, const CSTRING *fname_cstr, const UINT32 cdfs_md_id)
{
    ...
    cbytes_free(cbytes, LOC_CBITMAP_0008);
    return (EC_TRUE);
}
...
CBITMAP *cbitmap_dfs_load(const CSTRING *fname_cstr, const UINT32 cdfs_md_id)
{
    ...
    cbytes_free(cbytes, LOC_CBITMAP_0011);
    return (cbitmap);
}
