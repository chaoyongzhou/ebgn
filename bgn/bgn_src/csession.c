...
UINT32 csession_start()
{
    ...
    if(ERR_MODULE_ID == csession_md_id)
    {
        return (ERR_MODULE_ID);
    }

    /* initilize new one CSESSION module */
...
void csession_end(const UINT32 csession_md_id)
{
    ...
    clist_clean_with_modi(CSESSION_MD_SESSION_LIST(csession_md), csession_md_id, (CLIST_DATA_MODI_CLEANER)csession_node_free);
...
static UINT32 __csession_reserve_id(const UINT32 csession_md_id)
{
    ...
    {
        sys_log(LOGSTDOUT,
                "error:__csession_reserve_id: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
...
CSESSION_NODE *csession_node_new(const UINT32 csession_md_id, const CSTRING *name, const UINT32 expire_nsec)
{
    CSESSION_NODE *csession_node;
    ...
    {
        sys_log(LOGSTDOUT,
                "error:csession_node_new: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
...
EC_BOOL csession_node_init(const UINT32 csession_md_id, CSESSION_NODE *csession_node)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_node_init: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
...
EC_BOOL csession_node_clean(const UINT32 csession_md_id, CSESSION_NODE *csession_node)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_node_clean: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
    ...
    clist_clean_with_modi(CSESSION_NODE_CACHE_TREE(csession_node), csession_md_id, (CLIST_DATA_MODI_CLEANER)csession_item_free);
    cstring_clean(CSESSION_NODE_NAME(csession_node));
    CSESSION_NODE_CLEAN_ACCESS_CMUTEX(csession_node, LOC_CSESSION_0014);
    return (EC_TRUE);
}

EC_BOOL csession_node_free(const UINT32 csession_md_id, CSESSION_NODE *csession_node)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_node_free: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
    ...
    if(NULL_PTR != csession_node)
    {
        csession_node_clean(csession_md_id, csession_node);
        free_static_mem(MD_CSESSION, csession_md_id, MM_CSESSION_NODE, csession_node, LOC_CSESSION_0015);
    }
    return (EC_TRUE);
}
...
CSESSION_ITEM *csession_item_new(const UINT32 csession_md_id, const CSTRING *key, const CBYTES *val)
{
    ...
    {
        sys_log(LOGSTDOUT,
                "error:csession_item_new: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
...
EC_BOOL csession_item_init(const UINT32 csession_md_id, CSESSION_ITEM *csession_item)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_item_init: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
...
EC_BOOL csession_item_clean(const UINT32 csession_md_id, CSESSION_ITEM *csession_item)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_item_clean: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
    ...
    cbytes_clean(CSESSION_ITEM_VAL(csession_item), LOC_CSESSION_0019);

    clist_clean_with_modi(CSESSION_ITEM_CHILDREN(csession_item), csession_md_id, (CLIST_DATA_MODI_CLEANER)csession_item_free);

    return (EC_TRUE);
}

EC_BOOL csession_item_free(const UINT32 csession_md_id, CSESSION_ITEM *csession_item)
{
#if ( SWITCH_ON == CSESSION_DEBUG_SWITCH )
    if ( CSESSION_MD_ID_CHECK_INVALID(csession_md_id) )
    {
        sys_log(LOGSTDOUT,
                "error:csession_item_free: csession module #0x%lx not started.\n",
                csession_md_id);
        csession_print_module_status(csession_md_id, LOGSTDOUT);
        dbg_exit(MD_CSESSION, csession_md_id);
    }
    ...
    if(NULL_PTR != csession_item)
    {
        csession_item_clean(csession_md_id, csession_item);
        free_static_mem(MD_CSESSION, csession_md_id, MM_CSESSION_ITEM, csession_item, LOC_CSESSION_0020);
    }
...
void csession_item_print(LOG *log, const CSESSION_ITEM *csession_item, const UINT32 level)
{
    ...
    CLIST_LOCK(CSESSION_ITEM_CHILDREN(csession_item), LOC_CSESSION_0021);
...
CSESSION_NODE *csession_search_by_id(const UINT32 csession_md_id, const UINT32 session_id)
{
    ...
    csession_node = (CSESSION_NODE *)CLIST_DATA_DATA(clist_data);
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0024);
    return (csession_node);
}
...
EC_BOOL csession_add(const UINT32 csession_md_id, const CSTRING *name, const UINT32 expire_nsec)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0025);
    ...
    csession_node = csession_node_new(csession_md_id, name, expire_nsec);
    if(NULL_PTR == csession_node)
    {
    ...
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0028);
    return (EC_TRUE);
}
...
EC_BOOL csession_rmv_by_name(const UINT32 csession_md_id, const CSTRING *name)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0029);
...
EC_BOOL csession_rmv_by_id(const UINT32 csession_md_id, const UINT32 session_id)
{
    ...
    csession_md = CSESSION_MD_GET(csession_md_id);
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0032);
...
EC_BOOL csession_rmv_by_name_regex(const UINT32 csession_md_id, const CSTRING *session_name_regex)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0035);

    CLIST_LOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0036);
    CLIST_LOOP_NEXT(CSESSION_MD_SESSION_LIST(csession_md), clist_data)
    {
        CSESSION_NODE *csession_node;
    ...
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0038);
    pcre_free(name_re);
...
EC_BOOL csession_rmv_by_id_regex(const UINT32 csession_md_id, const CSTRING *session_id_regex)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0039);

    CLIST_LOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0040);
    CLIST_LOOP_NEXT(CSESSION_MD_SESSION_LIST(csession_md), clist_data)
    {
        CSESSION_NODE *csession_node;
    ...
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0042);
    pcre_free(id_re);
...
EC_BOOL csession_get_id(const UINT32 csession_md_id, const CSTRING *session_name, UINT32 *session_id)
{
    ...
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0044);
    return (EC_TRUE);
}
...
EC_BOOL csession_set(const UINT32 csession_md_id, CSESSION_NODE *csession_node, const CSTRING *path, const CBYTES *val)
{
    ...
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0047);
    return (EC_TRUE);
}
...
EC_BOOL csession_set_by_name(const UINT32 csession_md_id, const CSTRING *session_name, const CSTRING *path, const CBYTES *val)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0048);
...
EC_BOOL csession_set_by_id(const UINT32 csession_md_id, const UINT32 session_id, const CSTRING *path, const CBYTES *val)
{
    ...
    CSESSION_MD_CRWLOCK_WRLOCK(csession_md, LOC_CSESSION_0052);
    ...
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0055);
    return (EC_TRUE);
}
                                            ...
                                            CLIST *csession_item_list)
{
    ...
    CLIST_LOCK(cache_tree, LOC_CSESSION_0056);
                                            ...
                                            CLIST *csession_item_list)
{
    ...
    CLIST_LOCK(cache_tree, LOC_CSESSION_0060);
    CLIST_LOOP_NEXT(cache_tree, clist_data)
    {
        CSESSION_ITEM      *csession_item;
        CSESSION_ITEM      *new_csession_item;
    ...
    CLIST_UNLOCK(cache_tree, LOC_CSESSION_0063);
...
EC_BOOL csession_get_by_name(const UINT32 csession_md_id, const CSTRING *session_name, const CSTRING *path, CLIST *csession_item_list)
{
    ...
    CSESSION_MD_CRWLOCK_RDLOCK(csession_md, LOC_CSESSION_0066);

    csession_node = csession_search_by_name(csession_md_id, session_name);
    if(NULL_PTR == csession_node)
    {
        CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0067);
        return (EC_TRUE);
    }
    ...
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0069);
...
EC_BOOL csession_get_by_id(const UINT32 csession_md_id, const UINT32 session_id, const CSTRING *path, CLIST *csession_item_list)
{
    ...
    CSESSION_MD_CRWLOCK_RDLOCK(csession_md, LOC_CSESSION_0071);

    csession_node = csession_search_by_id(csession_md_id, session_id);
    if(NULL_PTR == csession_node)
    {
        CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0072);
        return (EC_TRUE);
    }
    ...
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0074);
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0075);
    return (EC_TRUE);
}
                                            ...
                                            CLIST *csession_item_list)
{
    ...
    if(seg_idx >= seg_num)
    {
        CLIST_LOCK(cache_tree, LOC_CSESSION_0076);
        CLIST_LOOP_NEXT(cache_tree, clist_data)
        {
            CSESSION_ITEM      *csession_item;
            CSESSION_ITEM      *new_csession_item;
    ...
    CLIST_LOCK(cache_tree, LOC_CSESSION_0079);
    CLIST_LOOP_NEXT(cache_tree, clist_data)
    {
        CSESSION_ITEM      *csession_item;
        CSESSION_ITEM      *new_csession_item;
    ...
    CLIST_UNLOCK(cache_tree, LOC_CSESSION_0082);
...
EC_BOOL csession_get_children_by_name(const UINT32 csession_md_id, const CSTRING *session_name, const CSTRING *path, CLIST *csession_item_list)
{
    ...
    CSESSION_MD_CRWLOCK_RDLOCK(csession_md, LOC_CSESSION_0084);

    csession_node = csession_search_by_name(csession_md_id, session_name);
    if(NULL_PTR == csession_node)
    {
        CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0085);
        return (EC_TRUE);
    }
    ...
    CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0087);
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0088);
    return (EC_TRUE);
}
...
EC_BOOL csession_get_by_name_regex(const UINT32 csession_md_id, const CSTRING *session_name_regex, const CSTRING *path, CLIST *csession_node_list)
{
    ...
    CSESSION_MD_CRWLOCK_RDLOCK(csession_md, LOC_CSESSION_0089);

    CLIST_LOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0090);
    CLIST_LOOP_NEXT(CSESSION_MD_SESSION_LIST(csession_md), clist_data)
    {
        CSESSION_NODE *csession_node;
        CSTRING *csession_node_name;
        ...
        if(EC_FALSE == csession_get_key_regex(csession_md_id, csession_node, path, CSESSION_NODE_CACHE_TREE(new_csession_node)))
        {
            ...
            csession_node_free(csession_md_id, new_csession_node);
            CLIST_UNLOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0093);
            CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0094);
            pcre_free(name_re);
            return (EC_FALSE);
        }
        ...
        CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0095);
    }
    CLIST_UNLOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0096);
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0097);
    pcre_free(name_re);
...
EC_BOOL csession_get_by_id_regex(const UINT32 csession_md_id, const CSTRING *session_id_regex, const CSTRING *path, CLIST *csession_node_list)
{
    ...
    CSESSION_MD_CRWLOCK_RDLOCK(csession_md, LOC_CSESSION_0098);

    CLIST_LOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0099);
    CLIST_LOOP_NEXT(CSESSION_MD_SESSION_LIST(csession_md), clist_data)
    {
        CSESSION_NODE *csession_node;
        char csession_node_id_str[64];
        ...
        if(EC_FALSE == csession_get_key_regex(csession_md_id, csession_node, path, CSESSION_NODE_CACHE_TREE(new_csession_node)))
        {
            ...
            csession_node_free(csession_md_id, new_csession_node);
            CLIST_UNLOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0102);
            CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0103);
            pcre_free(id_re);
            return (EC_FALSE);
        }
        ...
        CSESSION_NODE_UPDATE_ACCESS_TIME(csession_node, LOC_CSESSION_0104);
    }
    CLIST_UNLOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0105);
    CSESSION_MD_CRWLOCK_UNLOCK(csession_md, LOC_CSESSION_0106);
...
EC_BOOL csession_expire_handle(const UINT32 csession_md_id)
{
    ...
    CLIST_LOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0107);
    CLIST_LOOP_NEXT(CSESSION_MD_SESSION_LIST(csession_md), clist_data)
    {
        CSESSION_NODE *csession_node;
        CLIST_DATA    *clist_data_rmv;
    ...
    CLIST_UNLOCK(CSESSION_MD_SESSION_LIST(csession_md), LOC_CSESSION_0108);
