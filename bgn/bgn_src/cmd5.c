...
CMD5_DIGEST *cmd5_digest_new()
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CMD5_DIGEST, &cmd5_digest, LOC_CMD5_0001);
...
EC_BOOL cmd5_digest_free(CMD5_DIGEST *cmd5_digest)
{
    if(NULL_PTR != cmd5_digest)
    {
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CMD5_DIGEST, cmd5_digest, LOC_CMD5_0002);
    }
    return (EC_TRUE);
}
...
EC_BOOL cmd5_digest_init_0(const UINT32 md_id, CMD5_DIGEST *cmd5_digest)
{
    return cmd5_digest_init(cmd5_digest);
}

EC_BOOL cmd5_digest_clean_0(const UINT32 md_id, CMD5_DIGEST *cmd5_digest)
{
    return cmd5_digest_clean(cmd5_digest);
}

EC_BOOL cmd5_digest_free_0(const UINT32 md_id, CMD5_DIGEST *cmd5_digest)
{
    return cmd5_digest_free(cmd5_digest);
}
