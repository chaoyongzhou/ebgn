...
CBUFFER* cbuffer_new(const uint32_t size) 
{
    ...
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBUFFER, &cbuffer, LOC_CBUFFER_0001);
    if(NULL_PTR == cbuffer)
    {
    ...
    if(EC_FALSE == cbuffer_init(cbuffer, size))
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBUFFER, cbuffer, LOC_CBUFFER_0002);
        return (NULL_PTR);
    }
...
EC_BOOL cbuffer_free(CBUFFER *cbuffer) 
{
    if(NULL_PTR != cbuffer)
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBUFFER, cbuffer, LOC_CBUFFER_0005);
    }
    return (EC_TRUE);
}

EC_BOOL cbuffer_init_0(const UINT32 md_id, CBUFFER *cbuffer) 
{
    return cbuffer_init(cbuffer, 0);
}

EC_BOOL cbuffer_clean_0(const UINT32 md_id, CBUFFER *cbuffer) 
{
    return cbuffer_clean(cbuffer);
}

EC_BOOL cbuffer_free_0(const UINT32 md_id, CBUFFER *cbuffer) 
{
    return cbuffer_free(cbuffer);
}
