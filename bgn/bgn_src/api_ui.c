...
void api_ui_task0 ()
{
    ...
    int   ui_command_length; /* len of user input string */
    int   ui_arg_length;     /* size of thing between spaces on user input */
    ...
    for(;;)
    {
        ...
        /* len to process minus leading spaces */
        ui_command_length = strlen(ui_command) - strspn(ui_command," ");
        ...
        ui_argc = 0;
        ptr = (char *)ui_command;
        ptr += strspn(ui_command," ");         /* point past leading spaces */
        while (ui_command_length > 0 )
        {
            ui_arg_length = strcspn(ptr," ");  /* size to next space */
            ui_argv[ui_argc++] = ptr;         /* set argv to point to string location */
            ptr += ui_arg_length;      /* update pointer */
            *ptr++ = '\0';              /* and null terminate */
            /* subtract word, null, spaces */
            ui_command_length -= (ui_arg_length + 1 + strspn(ptr," "));
            ptr += strspn(ptr," ");             /* update pointer past any spaces */
        }
...
void api_ui_add_to_new_node_list(API_UI_CNODE** start_ptr, API_UI_NODE** node_ptr)
{
    ...
    (*start_ptr) = (API_UI_CNODE*) api_ui_malloc(sizeof(API_UI_CNODE), LOC_API_0413);
...
API_UI_ERR api_ui_cleanup(API_UI_ERR err, char* copy_cmd_str, API_UI_CNODE* new_node_list)
{
    ...
    while(new_node_list != NULL_PTR)
    {
        ...
        api_ui_free(tmp_ptr, LOC_API_0415);
    }

    /* Free the copy of the command string */
    api_ui_free(copy_cmd_str, LOC_API_0416);
...
API_UI_ELEM* api_ui_create_elem(const char* word, const char* help, API_UI_ELEM_TYPE type)
{
    ...
    copy_word = (char*) api_ui_malloc(sizeof(char) * size, LOC_API_0417);
    ...
    returnValue = (API_UI_ELEM*) api_ui_malloc(sizeof(API_UI_ELEM), LOC_API_0418);
...
API_UI_NODE* api_ui_create_node(API_UI_ELEM* element, API_UI_SECURITY_LEVEL sl)
{
    ...
    new_node = (API_UI_NODE*)api_ui_malloc(sizeof(API_UI_NODE), LOC_API_0419);
...
{
    api_ui_free((void*)element->word, LOC_API_0420);
    api_ui_free(element, LOC_API_0421);
}
...
API_UI_ERR api_ui_common_define (API_UI_SECURITY_LEVEL sl, API_UI_HANDLER handler, const char* help_str, const char* cmd_str, va_list params)
{
    ...
    /* Copy the cmd_str */
    copy = (char*) api_ui_malloc(sizeof(char) * (strlen(cmd_str) + 1), LOC_API_0422);
