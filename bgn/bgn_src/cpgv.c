...
static CPGV_HDR *__cpgv_hdr_load(const UINT32 fsize, int fd)
{
    uint8_t *buff;
    UINT32   offset;

    buff = (uint8_t *)safe_malloc(fsize, LOC_CPGV_0004);
    if(NULL_PTR == buff)
    {
...
static CPGV_HDR * __cpgv_hdr_flush(CPGV_HDR *cpgv_hdr, const UINT32 fsize, int fd)
{   
    ...
    {   
        ...
        offset = 0;        
        if(EC_FALSE == c_file_flush(fd, &offset, fsize, (const UINT8 *)cpgv_hdr))
        {
...
static CPGV_HDR *__cpgv_hdr_free(CPGV_HDR *cpgv_hdr, const UINT32 fsize, int fd)
{   
    if(NULL_PTR != cpgv_hdr)
    {   
        UINT32 offset;
        ...
        safe_free(cpgv_hdr, LOC_CPGV_0006);
    }
...
static CPGV_HDR *__cpgv_hdr_new(CPGV *cpgv)
{
    ...
    cpgv_hdr = (CPGV_HDR *)safe_malloc(CPGV_FSIZE(cpgv), LOC_CPGV_0007);
    if(NULL_PTR == cpgv_hdr)
    {
    ...
    if(EC_FALSE == cpgv_hdr_init(cpgv))
    {
        ...
        safe_free(cpgv_hdr, LOC_CPGV_0008);
...
CPGV_HDR *cpgv_hdr_create(CPGV *cpgv)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_create: mmap file %s failed, errno = %d, errorstr = %s\n", 
...
CPGV_HDR *cpgv_hdr_create_1(CPGV *cpgv)
{
    CPGV_HDR *cpgv_hdr;

    ASSERT_CPGV_HDR_PAD_SIZE();

    cpgv_hdr = __cpgv_hdr_new(cpgv);
    if(NULL_PTR == cpgv_hdr)
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_create: new cpgv header failed, try to mmap\n");
    ...
    __cpgv_hdr_free(cpgv_hdr, CPGV_FSIZE(cpgv), CPGV_FD(cpgv));
    CPGV_HEADER(cpgv) = NULL_PTR;
...
EC_BOOL cpgv_hdr_init(CPGV *cpgv)
{
    ...
    /*statistics*/    
    CPGV_HDR_PAGE_4K_MAX_NUM(cpgv_hdr)       = 0;
...
CPGV_HDR *cpgv_hdr_open(CPGV *cpgv)
{
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_hdr_open: fsize %u\n", CPGV_FSIZE(cpgv));

    cpgv_hdr = (CPGV_HDR *)mmap(NULL_PTR, CPGV_FSIZE(cpgv), PROT_READ | PROT_WRITE, MAP_SHARED, CPGV_FD(cpgv), 0);
    if(MAP_FAILED == cpgv_hdr)
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_open: mmap file %s with fd %d failed, errno = %d, errorstr = %s\n", 
...
{   
    ...
    {   
        ...
        {
            dbg_log(SEC_0074_CPGV, 1)(LOGSTDOUT, "warn:cpgv_hdr_close: sync cpgv_hdr of %s with size %u failed\n", 
                               CPGV_FNAME(cpgv), CPGV_FSIZE(cpgv));
        }
        
        if(0 != munmap(CPGV_HEADER(cpgv), CPGV_FSIZE(cpgv)))
        {
            dbg_log(SEC_0074_CPGV, 1)(LOGSTDOUT, "warn:cpgv_hdr_close: munmap cpgv of %s with size %u failed\n", 
                               CPGV_FNAME(cpgv), CPGV_FSIZE(cpgv));
        }
...
{   
    ...
    {   
        if(0 != msync(CPGV_HEADER(cpgv), CPGV_FSIZE(cpgv), MS_SYNC))
        {
            dbg_log(SEC_0074_CPGV, 1)(LOGSTDOUT, "warn:cpgv_hdr_sync: sync cpgv_hdr of %s with size %u failed\n", 
                               CPGV_FNAME(cpgv), CPGV_FSIZE(cpgv));
        }        
    }
...
EC_BOOL cpgv_hdr_flush(const CPGV_HDR *cpgv_hdr, int fd, UINT32 *offset)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_flush: flush CPGV_HDR_PAGE_4K_MAX_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_flush: flush CPGV_HDR_PAGE_4K_USED_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }    
...
EC_BOOL cpgv_hdr_load(CPGV_HDR *cpgv_hdr, int fd, UINT32 *offset)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_load: load CPGV_HDR_PAGE_4K_MAX_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }    
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_hdr_load: load CPGV_HDR_PAGE_4K_USED_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }
...
CPGV *cpgv_new(const uint8_t *cpgv_fname)
{
    ...
    alloc_static_mem(MD_TBD, 0, MM_CPGV, &cpgv, LOC_CPGV_0009);
    if(NULL_PTR == cpgv)
    {
...
EC_BOOL cpgv_free(CPGV *cpgv)
{
    if(NULL_PTR != cpgv)
    {
        ...
        free_static_mem(MD_TBD, 0, MM_CPGV, cpgv, LOC_CPGV_0011);
    }
...
CPGV *cpgv_open0(const uint8_t *cpgv_fname)
{
    ...
    uint64_t     page_4k_max_num; /*max pages number */
    uint64_t     page_4k_used_num;/*used pages number*/
    ...
    alloc_static_mem(MD_TBD, 0, MM_CPGV, &cpgv, LOC_CPGV_0012);
    if(NULL_PTR == cpgv)
    {
    ...
    for(disk_no = 0; disk_no < CPGV_MAX_DISK_NUM; disk_no ++)
    {
        ...
        disk_num ++;
        page_4k_max_num  += CPGD_PAGE_4K_MAX_NUM(cpgd);
        page_4k_used_num += CPGD_PAGE_4K_USED_NUM(cpgd);
        actual_used_size += CPGD_PAGE_ACTUAL_USED_SIZE(cpgd);        
    }

    /*update cpgv statistics info*/
    CPGV_PAGE_DISK_NUM(cpgv)         = disk_num;
    CPGV_PAGE_4K_MAX_NUM(cpgv)       = page_4k_max_num;
    CPGV_PAGE_4K_USED_NUM(cpgv)      = page_4k_used_num;
...
CPGV *cpgv_open(const uint8_t *cpgv_fname)
{
    ...
    alloc_static_mem(MD_TBD, 0, MM_CPGV, &cpgv, LOC_CPGV_0013);
    if(NULL_PTR == cpgv)
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_open:malloc cpgv failed\n");
        return (NULL_PTR);
...
EC_BOOL cpgv_close(CPGV *cpgv)
{
    if(NULL_PTR != cpgv)
    {
        ...
        free_static_mem(MD_TBD, 0, MM_CPGV, cpgv, LOC_CPGV_0015);
    }
    return (EC_TRUE);
}
...
EC_BOOL cpgv_add_disk(CPGV *cpgv, const uint16_t disk_no)
{
    ...
    /*statistics*/    
    CPGV_PAGE_4K_MAX_NUM(cpgv)       += ((uint64_t)1) * CPGD_MAX_BLOCK_NUM * CPGD_BLOCK_PAGE_NUM;
    CPGV_PAGE_4K_USED_NUM(cpgv)      += 0;
    CPGV_PAGE_ACTUAL_USED_SIZE(cpgv) += 0;
...
EC_BOOL cpgv_del_disk(CPGV *cpgv, const uint16_t disk_no)
{
    ...
    /*statistics*/    
    CPGV_PAGE_4K_MAX_NUM(cpgv)       -= ((uint64_t)1) * CPGD_MAX_BLOCK_NUM * CPGD_BLOCK_PAGE_NUM;;
    CPGV_PAGE_4K_USED_NUM(cpgv)      -= CPGD_PAGE_4K_USED_NUM(cpgd);
...
EC_BOOL cpgv_mount_disk(CPGV *cpgv, const uint16_t disk_no)
{
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_mount_disk: ### set to CPGD_MAX_BLOCK_NUM %d for debug purpose \n", CPGD_MAX_BLOCK_NUM);
    ...
    /*statistics*/    
    CPGV_PAGE_4K_MAX_NUM(cpgv)       += CPGD_PAGE_4K_MAX_NUM(cpgd);
    CPGV_PAGE_4K_USED_NUM(cpgv)      += CPGD_PAGE_4K_USED_NUM(cpgd);
    CPGV_PAGE_ACTUAL_USED_SIZE(cpgv) += CPGD_PAGE_ACTUAL_USED_SIZE(cpgd);
...
static EC_BOOL __cpgv_size_to_page_model(const uint32_t size, uint16_t *page_num, uint16_t *page_model)
{
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] __cpgv_size_to_page_model: size = %u ==> page_4k_num_need = %u\n", size, page_4k_num_need);
    ...
    for(t = page_4k_num_need, page_model_t = 0/*CPGB_064MB_MODEL*/; 0 == (t & e); t <<= 1, page_model_t ++)
    {
        /*do nothing*/
    }
...
EC_BOOL cpgv_new_space_from_disk(CPGV *cpgv, const uint32_t size, const uint16_t disk_no, uint16_t *block_no, uint16_t *page_4k_no)
{
    ...
    __cpgv_size_to_page_model(size, &page_4k_num_need, &page_model);

    //dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_new_space_from_disk: size = %u ==> page_4k_num_need = %u ==> page_model = %u (has %u 4k-pages )\n", 
    //                   size, page_4k_num_need, page_model, (uint16_t)(1 << (CPGB_MODEL_NUM - 1 - page_model)));
    ...
    {        
        dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] cpgv_new_space_from_disk: before delete disk_no_t %u: pgb bitmap %s, pgv assign bitmap %s\n", 
                            disk_no_t, 
        ...
        dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] cpgv_new_space_from_disk: after  delete disk_no_t %u: pgb bitmap %s, pgv assign bitmap %s\n", 
                            disk_no_t, 
                            c_uint16_to_bin_str(CPGD_PAGE_MODEL_ASSIGN_BITMAP(cpgd)), 
                            c_uint16_to_bin_str(CPGV_PAGE_MODEL_ASSIGN_BITMAP(cpgv)));

        dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] cpgv_new_space_from_disk: disk_no_t %u: max pages %u, used pages %u\n", 
        ...
        {   
            ...
            dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] cpgv_new_space_from_disk: page_model %u, page_model_t %u\n", page_model, page_model_t);
            /*add the disk into pool*/
            __cpgv_add_disk(cpgv, disk_no_t, page_model_t);
            dbg_log(SEC_0074_CPGV, 9)(LOGSTDNULL, "[DEBUG] cpgv_new_space_from_disk: disk_no_t %u: pgb bitmap %s, pgv assign bitmap %s\n", 
                                disk_no_t, 
                                c_uint16_to_bin_str(CPGD_PAGE_MODEL_ASSIGN_BITMAP(cpgd)), 
                                c_uint16_to_bin_str(CPGV_PAGE_MODEL_ASSIGN_BITMAP(cpgv)));
        }
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_new_space_from_disk: pgv_page_4k_used_num %u due to increment %u\n", 
                        CPGV_PAGE_4K_USED_NUM(cpgv), page_4k_num_need);
...
EC_BOOL cpgv_new_space(CPGV *cpgv, const uint32_t size, uint16_t *disk_no, uint16_t *block_no, uint16_t *page_4k_no)
{
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_new_space: size = %u ==> page_4k_num_need = %u ==> page_model = %u (has %u 4k-pages )\n", 
                       size, page_4k_num_need, page_model, (uint16_t)(1 << (CPGB_MODEL_NUM - 1 - page_model)));
    ...
    for(;;)/*Oops! fix inconsistency between cpgv and cpgd*/
    {
        ...
        if(EC_TRUE == cpgd_new_space(cpgd, size, &block_no_t, &page_4k_no_t))
        {
            page_model = page_model_t;
            break;
        }
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_new_space: pgv_page_4k_used_num %u due to increment %u\n", 
                        CPGV_PAGE_4K_USED_NUM(cpgv), page_4k_num_need);
...
EC_BOOL cpgv_free_space(CPGV *cpgv, const uint16_t disk_no, const uint16_t block_no, const uint16_t page_4k_no, const uint32_t size)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_free_space: disk_no %u free space of block_no %u, page_4k_no %u, size %u failed\n", 
    ...
    dbg_log(SEC_0074_CPGV, 9)(LOGSTDOUT, "[DEBUG] cpgv_free_space: pgv_page_4k_used_num %u due to decrement %u\n", 
                        CPGV_PAGE_4K_USED_NUM(cpgv), page_4k_num_used);
...
EC_BOOL cpgv_check(const CPGV *cpgv)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_check: inconsistent 4k-page max num: pgv_page_4k_max_num = %llu, pgd_page_4k_max_num = %llu\n",
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDOUT, "error:cpgv_check: inconsistent 4k-page used num: pgv_page_4k_used_num = %llu, pgd_page_4k_used_num = %llu\n",
...
EC_BOOL cpgv_debug_cmp(const CPGV *cpgv_1st, const CPGV *cpgv_2nd)
{
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDERR, "error:cpgv_debug_cmp: inconsistent CPGV_PAGE_4K_MAX_NUM: %u != %u\n", 
                            CPGV_PAGE_4K_MAX_NUM(cpgv_1st), CPGV_PAGE_4K_MAX_NUM(cpgv_2nd));
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0074_CPGV, 0)(LOGSTDERR, "error:cpgv_debug_cmp: inconsistent CPGV_PAGE_4K_USED_NUM: %u != %u\n", 
