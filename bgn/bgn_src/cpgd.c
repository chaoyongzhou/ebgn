...
CPGD_HDR *cpgd_hdr_mem_new(CPGD *cpgd, const uint16_t block_num)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_mem_new: malloc %ld bytes failed, errno = %d, errorstr = %s\n", 
                           CPGD_FSIZE(cpgd));
        return (NULL_PTR);
    }
...
CPGD_HDR *cpgd_hdr_new(CPGD *cpgd, const uint16_t block_num)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_new: mmap file %s failed, errno = %d, errorstr = %s\n", 
...
CPGD_HDR *cpgd_hdr_open(CPGD *cpgd)
{
    ...
    dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_hdr_open: fsize %u\n", CPGD_FSIZE(cpgd));

    cpgd_hdr = (CPGD_HDR *)mmap(NULL_PTR, CPGD_FSIZE(cpgd), PROT_READ | PROT_WRITE, MAP_SHARED, CPGD_FD(cpgd), 0);
    if(MAP_FAILED == cpgd_hdr)
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_open: mmap file %s with fd %d failed, errno = %d, errorstr = %s\n", 
...
{   
    ...
    {   
        if(0 != msync(CPGD_HEADER(cpgd), CPGD_FSIZE(cpgd), MS_SYNC))
        {
            dbg_log(SEC_0041_CPGD, 1)(LOGSTDOUT, "warn:cpgd_hdr_close: sync cpgd_hdr of %s with size %u failed\n", 
                               CPGD_FNAME(cpgd), CPGD_FSIZE(cpgd));
        }
        
        if(0 != munmap(CPGD_HEADER(cpgd), CPGD_FSIZE(cpgd)))
        {
            dbg_log(SEC_0041_CPGD, 1)(LOGSTDOUT, "warn:cpgd_hdr_close: munmap cpgd of %s with size %u failed\n", 
                               CPGD_FNAME(cpgd), CPGD_FSIZE(cpgd));
        }
...
{   
    ...
    {   
        if(0 != msync(CPGD_HEADER(cpgd), CPGD_FSIZE(cpgd), MS_SYNC))
        {
            dbg_log(SEC_0041_CPGD, 1)(LOGSTDOUT, "warn:cpgd_hdr_sync: sync cpgd_hdr of %s with size %u failed\n", 
                               CPGD_FNAME(cpgd), CPGD_FSIZE(cpgd));
        }             
    }
...
EC_BOOL cpgd_hdr_flush(const CPGD_HDR *cpgd_hdr, int fd, UINT32 *offset)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_flush: flush CPGD_HDR_PAGE_4K_MAX_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_flush: flush CPGD_HDR_PAGE_4K_USED_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }    
...
EC_BOOL cpgd_hdr_load(CPGD_HDR *cpgd_hdr, int fd, UINT32 *offset)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_load: load CPGD_HDR_PAGE_4K_MAX_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }    
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_hdr_load: load CPGD_HDR_PAGE_4K_USED_NUM at offset %u of fd %d failed\n", (*offset), fd);
        return (EC_FALSE);
    }
...
CPGD *cpgd_open(const uint8_t *cpgd_fname)
{
    ...
    alloc_static_mem(MD_TBD, 0, MM_CPGD, &cpgd, LOC_CPGD_0007);
    if(NULL_PTR == cpgd)
    {
...
EC_BOOL cpgd_close(CPGD *cpgd)
{
    if(NULL_PTR != cpgd)
    {
        ...
        if(NULL_PTR != CPGD_FNAME(cpgd))
        {
            safe_free(CPGD_FNAME(cpgd), LOC_CPGD_0008);
            CPGD_FNAME(cpgd) = NULL_PTR;
        }

        free_static_mem(MD_TBD, 0, MM_CPGD, cpgd, LOC_CPGD_0009);
    }
    return (EC_TRUE);
}
...
void cpgd_clean(CPGD *cpgd)
{
    ...
    if(NULL_PTR != CPGD_FNAME(cpgd))
    {
        safe_free(CPGD_FNAME(cpgd), LOC_CPGD_0010);
        CPGD_FNAME(cpgd) = NULL_PTR;
    }
    ...
    for(block_no = 0; block_no < CPGD_PAGE_BLOCK_MAX_NUM(cpgd); block_no ++)
    {
        ...
        {           
            safe_free(CPGD_BLOCK_CPGB(cpgd, block_no), LOC_CPGD_0011);
            CPGD_BLOCK_CPGB(cpgd, block_no) = NULL_PTR;
        }
    }  
    ...
    safe_free(CPGD_HEADER(cpgd), LOC_CPGD_0012);
    CPGD_HEADER(cpgd) = NULL_PTR;
...
EC_BOOL cpgd_new_space(CPGD *cpgd, const uint32_t size, uint16_t *block_no, uint16_t *page_4k_no)
{
    ...
    //dbg_log(SEC_0041_CPGD, 9)(LOGSTDNULL, "[DEBUG] cpgd_new_space: size = %u ==> page_4k_num_need = %u\n", size, page_4k_num_need);
    ...
    //dbg_log(SEC_0041_CPGD, 9)(LOGSTDNULL, "[DEBUG] cpgd_new_space: page_4k_num_need = %u ==> page_model = %u (has %u 4k-pages )\n", 
    ...
    for(;;)/*Oops! fix inconsistency between cpgd and cpgb*/
    {
        ...
        if(EC_TRUE == cpgb_new_space(cpgb, size, &page_4k_no_t))
        {
    ...
    dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_new_space: pgd_page_4k_used_num %u due to increment %u\n", 
                        CPGD_PAGE_4K_USED_NUM(cpgd), page_4k_num_need);
...
EC_BOOL cpgd_free_space(CPGD *cpgd, const uint16_t block_no, const uint16_t page_4k_no, const uint32_t size)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_free_space: block_no %u free space of page_4k_no %u, size %u failed\n", 
    ...
    dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_free_space: cpgd %p, block %u, asssign bitmap %s -> %s\n",
                       cpgd, block_no,
                       c_uint16_to_bin_str(pgb_assign_bitmap_old),
                       c_uint16_to_bin_str(pgb_assign_bitmap_new));
#endif
    if(pgb_assign_bitmap_new != pgb_assign_bitmap_old)
    {
        ...
        dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_free_space: cpgd %p, block %u, old asssign bitmap %s = page model %u\n",
                       cpgd, block_no,
                       c_uint16_to_bin_str(pgb_assign_bitmap_old), page_model_old);      

        dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_free_space: cpgd %p, block %u, new asssign bitmap %s = page model %u\n",
                       cpgd, block_no,
                       c_uint16_to_bin_str(pgb_assign_bitmap_new), page_model_new);                          
#endif
    ...
    dbg_log(SEC_0041_CPGD, 9)(LOGSTDOUT, "[DEBUG] cpgd_free_space: pgd_page_4k_used_num %u due to decrement %u\n", 
                        CPGD_PAGE_4K_USED_NUM(cpgd), page_4k_num_used);
...
EC_BOOL cpgd_load(CPGD *cpgd, int fd, UINT32 *offset)
{
    ...
    if(NULL_PTR == CPGD_HEADER(cpgd))
    {
        CPGD_HEADER(cpgd) = safe_malloc(sizeof(CPGD_HDR), LOC_CPGD_0013);
        if(NULL_PTR == CPGD_HEADER(cpgd))
        {
    ...
    for(block_no = 0; block_no < CPGD_PAGE_BLOCK_MAX_NUM(cpgd); block_no ++)
    {
        ...
        {
            CPGD_BLOCK_CPGB(cpgd, block_no) = safe_malloc(sizeof(CPGB), LOC_CPGD_0014);
            if(NULL_PTR == CPGD_BLOCK_CPGB(cpgd, block_no))
            {
...
EC_BOOL cpgd_check(const CPGD *cpgd)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_check: inconsistent 4k-page max num: pgd_page_4k_max_num = %u, pgb_page_4k_max_num = %u\n", 
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDOUT, "error:cpgd_check: inconsistent 4k-page used num: pgd_page_4k_used_num = %u, pgb_page_4k_used_num = %u\n", 
...
*/    
    sys_log(log, "cpgd_print: cpgd %p, block num %u, 4k-page max num %u, 4k-page used num %u, actual used size %llu, ratio %.2f\n", 
                 cpgd, 
...
EC_BOOL cpgd_debug_cmp(const CPGD *cpgd_1st, const CPGD *cpgd_2nd)
{
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDERR, "error:cpgd_debug_cmp: inconsistent CPGD_PAGE_4K_MAX_NUM: %u != %u\n", 
    ...
    {
        dbg_log(SEC_0041_CPGD, 0)(LOGSTDERR, "error:cpgd_debug_cmp: inconsistent CPGD_PAGE_4K_USED_NUM: %u != %u\n", 
...
CPGD *cpgd_mem_new(const uint16_t block_num)
{
    ...
    alloc_static_mem(MD_TBD, 0, MM_CPGD, &cpgd, LOC_CPGD_0015);
    if(NULL_PTR == cpgd)
    {
...
EC_BOOL cpgd_mem_free(CPGD *cpgd)
{
    if(NULL_PTR != cpgd)
    {
        ...
        free_static_mem(MD_TBD, 0, MM_CPGD, cpgd, LOC_CPGD_0016);
    }
