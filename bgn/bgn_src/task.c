...
UINT32 task_caller(TASK_FUNC *task_func, FUNC_ADDR_NODE *func_addr_node)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_caller: func_ret_val %lx\n", (UINT32)task_func->func_ret_val);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT,"task_caller: calling back\n");

    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT,"task_caller: leave\n");
...
EC_BOOL task_node_buff_alloc(TASK_NODE *task_node, const UINT32 buff_size)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_node_buff_alloc: buff_size = %ld, (KB: %ld, MB: %ld), buff_type = %ld\n", buff_size, buff_size/1024, buff_size/1024/1024, buff_type);
...
EC_BOOL task_node_buff_realloc(TASK_NODE *task_node, const UINT32 new_size)
{
    ...
    alloc_static_mem(MD_TASK, 0, new_type, &(new_buff), LOC_TASK_0003);
    if(NULL_PTR == TASK_NODE_BUFF(task_node))
    {
...
EC_BOOL task_node_buff_free(TASK_NODE *task_node)
{
    if(NULL_PTR != TASK_NODE_BUFF(task_node))
    {
        free_static_mem(MD_TASK, CMPI_ANY_MODI, TASK_NODE_BUFF_TYPE(task_node), TASK_NODE_BUFF(task_node), LOC_TASK_0004);
        TASK_NODE_BUFF(task_node)      = NULL_PTR;
        TASK_NODE_BUFF_TYPE(task_node) = MM_END;
        TASK_NODE_BUFF_LEN(task_node)  = 0;
        TASK_NODE_BUFF_POS(task_node)  = 0;
...
EC_BOOL task_node_free(TASK_NODE *task_node)
{
    if(NULL_PTR != task_node)
    {
        ...
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_TASK_NODE, task_node, LOC_TASK_0006);
    }
...
EC_BOOL task_req_clean(TASK_REQ *task_req)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 1)(LOGSTDNULL, "warn:task_req_clean: task_req_func is null\n");
        return (EC_TRUE);
    }

    if(NULL_PTR == func_addr_node)
    {
        dbg_log(SEC_0015_TASK, 1)(LOGSTDNULL, "warn:task_req_clean: func_addr_node is null\n");
        return (EC_TRUE);
    }
    ...
    if(e_dbg_void != func_addr_node->func_ret_type)
    {
        ...
        {
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(task_req_func->func_ret_val), LOC_TASK_0007);
    ...
    for( para_idx = 0; para_idx < TASK_REQ_FUNC_PARA_NUM(task_req) && para_idx < MAX_NUM_OF_FUNC_PARAS; para_idx ++ )
    {
        ...
        {
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(func_para->para_val), LOC_TASK_0008);
            func_para->para_val = 0;
        }
    }
    return (EC_TRUE);
...
EC_BOOL task_req_func_para_encode(const UINT32 comm, const UINT32 func_para_num, FUNC_PARA *func_para_tbl, const FUNC_ADDR_NODE *func_addr_node, UINT8 *out_buff, const UINT32 out_buff_max_len, UINT32 *position)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode: func_para_num = %ld, out_buff_max_len = %ld, position = %ld, func_addr_node belong to func id %lx\n", func_para_num, out_buff_max_len, *position, func_addr_node->func_index);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode: func_addr_node %lx:\n", func_addr_node);
    ...
    if(EMB_NUM_OF_FUNC_PARAS == func_para_num)
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode[0]: para_idx 0: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 0 ], func_para->para_val, *position);
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode[1]: para_idx 1: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 1 ], func_para->para_val, *position);
        /*retval is always E_DIRECT_OUT, so ignore during task_req encoding*/

        /*the 3rd para must be user interface/function id*/
        func_para = (func_para_tbl + 2);
        func_para->para_dir = func_addr_node->func_para_direction[ 2 ];
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode[2]: para_idx 2: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 2 ], func_para->para_val, *position);
        ui_func_id = func_para->para_val;
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode[2]: ui_func_id = %lx\n", ui_func_id);
        cmpi_encode_uint32(comm, (func_para->para_dir), out_buff, out_buff_max_len, position);
        type_conv_item = dbg_query_type_conv_item_by_type(func_addr_node->func_para_type[ 2 ]);
        if( NULL_PTR == type_conv_item )
        {
    ...
    for( para_idx = 0; para_idx < func_para_num; para_idx ++ )
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_encode[3]: para_idx = %ld, para_type = %ld, para_dir = %ld, position = %ld\n", para_idx, func_addr_node->func_para_type[ para_idx ], func_para->para_dir, *position);
        cmpi_encode_uint32(comm, (func_para->para_dir), out_buff, out_buff_max_len, position);
...
EC_BOOL task_req_func_para_decode(const UINT32 comm, const UINT8 *in_buff, const UINT32 in_buff_max_len, UINT32 *position, UINT32 *func_para_num, FUNC_PARA *func_para_tbl, const FUNC_ADDR_NODE *func_addr_node)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode: func_para_num = %lx, in_buff_max_len = %ld, position = %ld, func_addr_node belong to func id %lx\n", (*func_para_num), in_buff_max_len, *position, func_addr_node->func_index);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode: func_addr_node %lx:\n", func_addr_node);
    //func_addr_node_print(LOGSTDOUT, func_addr_node);
    ...
    if(EMB_NUM_OF_FUNC_PARAS == (*func_para_num))
    {
        ...
        if(E_DIRECT_IN == func_para->para_dir || E_DIRECT_IO == func_para->para_dir)
        {
            //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "xxx para_idx = %ld, pointer_flag = %ld, ap = %lx\n", para_idx, TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item), ap);
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode[0]: para_idx 0: para_dir = %ld, para_type = %ld, para_val = %lx, in_buff_max_len = %ld,position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 0 ], func_para->para_val, in_buff_max_len, *position);
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode[2]: para_idx 2: para_dir = %ld, para_type = %ld, para_val = %lx, in_buff_max_len = %ld,position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 2 ], func_para->para_val, in_buff_max_len, *position);
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode: ui_func_id %lx\n", ui_func_id);
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode: ui_func_addr_node %lx:\n", ui_func_addr_node);
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode[1]: para_idx 1: para_dir = %ld, para_type = %ld, para_val = %lx, in_buff_max_len = %ld,position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 1 ], func_para->para_val, in_buff_max_len, *position);
    ...
    for( para_idx = 0; para_idx < (*func_para_num); para_idx ++ )
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_func_para_decode[3]: para_idx = %ld, para_type = %ld, para_dir = %ld, in_buff_max_len = %ld,position = %ld\n", para_idx, func_addr_node->func_para_type[ para_idx ], func_para->para_dir, in_buff_max_len, *position);
        ...
        if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item))
        {
            alloc_static_mem(MD_TASK, 0, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void **)&(func_para->para_val), LOC_TASK_0012);
            dbg_tiny_caller(2, TYPE_CONV_ITEM_VAR_INIT_FUNC(type_conv_item), CMPI_ANY_MODI, func_para->para_val);
            ap = (void *)func_para->para_val;
        }
        else
...
EC_BOOL task_req_decode(const UINT32 recv_comm, TASK_REQ *task_req)
{
...
#if 0
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_decode: TASK_REQ_SEND_MOD:[0] ");
...
#if 0
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_decode: TASK_REQ_RECV_MOD:[0] ");
    ...
    if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item))
    {
        alloc_static_mem(MD_TASK, 0, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void **)&(task_req_func->func_ret_val), LOC_TASK_0013);
...
{
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "isend  req: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
...
#if 0
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "========================================= task_req_isend =========================================\n");
    task_req_print(LOGSTDOUT, task_req);
#endif
    return (EC_TRUE);
}
...
EC_BOOL task_req_md_mod_mgr_get(TASK_BRD *task_brd, TASK_REQ *task_req, MOD_MGR **mod_mgr)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_md_mod_mgr_get: md_type %ld, md_id %ld, md_mod_mgr %lx\n", md_type, md_id, md_mod_mgr);
...
EC_BOOL task_rsp_clean(TASK_RSP *task_rsp)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 1)(LOGSTDNULL, "warn:task_rsp_clean: task_rsp_func is null\n");
        return (EC_TRUE);
    }

    if(NULL_PTR == func_addr_node)
    {
        dbg_log(SEC_0015_TASK, 1)(LOGSTDNULL, "warn:task_rsp_clean: func_addr_node is null\n");
        return (EC_TRUE);
    }
    ...
    if(e_dbg_void != func_addr_node->func_ret_type)
    {
        ...
        {
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(task_rsp_func->func_ret_val), LOC_TASK_0014);
    ...
    for( para_idx = 0; para_idx < TASK_RSP_FUNC_PARA_NUM(task_rsp) && para_idx < MAX_NUM_OF_FUNC_PARAS; para_idx ++ )
    {
        ...
        if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item) && 0 != func_para->para_val)
        {
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)(func_para->para_val), LOC_TASK_0015);
            func_para->para_val = 0;
        }
    }
    return (EC_TRUE);
...
EC_BOOL task_rsp_func_para_encode(const UINT32 comm, const UINT32 func_para_num, FUNC_PARA *func_para_tbl, const FUNC_ADDR_NODE *func_addr_node, UINT8 *out_buff, const UINT32 out_buff_max_len, UINT32 *position)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode: func_para_num = %ld, out_buff_max_len = %ld, position = %ld, func_addr_node belong to func id %lx\n", func_para_num, out_buff_max_len, *position, func_addr_node->func_index);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode: func_addr_node %lx:\n", func_addr_node);
    //func_addr_node_print(LOGSTDOUT, func_addr_node);
    ...
    if(EMB_NUM_OF_FUNC_PARAS == func_addr_node->func_para_num)
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode: ui_func_id %lx\n", ui_func_id);
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode: ui_func_addr_node %lx:\n", ui_func_addr_node);
        //func_addr_node_print(LOGSTDOUT, ui_func_addr_node);

        /*the 1st para must be container module id*/
        func_para = (func_para_tbl + 0);
        /*ignore E_DIRECT_IN parameter encoding of task_rsp*/
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode[0]: para_idx 0: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 0 ], func_para->para_val, *position);

        /*the 2nd para must be user interface/function retval addr*/
        func_para = (func_para_tbl + 1);
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode[1]: para_idx 1: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 1 ], func_para->para_val, *position);
        if(e_dbg_void != ui_func_addr_node->func_ret_type)
        {
            type_conv_item = dbg_query_type_conv_item_by_type(ui_func_addr_node->func_ret_type);
            if( NULL_PTR == type_conv_item )
        ...
        func_para = (func_para_tbl + 2);
        /*ignore E_DIRECT_IN parameter encoding of task_rsp*/
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode[2]: para_idx 2: para_dir = %ld, para_type = %ld, para_val = %lx, position = %ld\n", func_para->para_dir, func_addr_node->func_para_type[ 2 ], func_para->para_val, *position);
    ...
    for( para_idx = 0; para_idx < func_para_num; para_idx ++ )
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_encode[3]: para_idx = %ld, para_type = %ld, para_dir = %ld, pointer_flag = %ld, position = %ld\n", para_idx, func_addr_node->func_para_type[ para_idx ], func_para->para_dir, TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item), *position);
        ...
        if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item) && 0 != func_para->para_val)
        {
            ...
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)func_para->para_val, LOC_TASK_0017);/*clean up*/
            func_para->para_val = 0;
        }
...
EC_BOOL task_rsp_func_para_decode0(const UINT32 comm, const UINT8 *in_buff, const UINT32 in_buff_max_len, UINT32 *position, const UINT32 func_para_num, FUNC_PARA *task_req_func_para_tbl, FUNC_PARA *task_rsp_func_para_tbl, const FUNC_ADDR_NODE *func_addr_node)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_decode: func_para_num = %lx, in_buff_max_len = %ld, position = %ld, func_addr_node belong to func id %lx\n", (func_para_num), in_buff_max_len, *position, func_addr_node->func_index);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_decode: func_addr_node %lx:\n", func_addr_node);
    //func_addr_node_print(LOGSTDOUT, func_addr_node);
    ...
    if(EMB_NUM_OF_FUNC_PARAS == func_addr_node->func_para_num)
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_decode[1]: para_idx 1: para_dir = %ld, para_type = %ld, para_val = %lx, in_buff_max_len = %ld,position = %ld\n", task_req_func_para->para_dir, ui_func_addr_node->func_para_type[ 1 ], task_req_func_para->para_val, in_buff_max_len, *position);
    ...
    for( para_idx = 0; para_idx < func_para_num; para_idx ++ )
    {
        ...
        //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_func_para_decode[3]: para_idx = %ld, para_type = %ld, para_dir = %ld, in_buff_max_len = %ld,position = %ld\n", para_idx, func_addr_node->func_para_type[ para_idx ], func_addr_node->func_para_direction[ para_idx ], in_buff_max_len, *position);
...
EC_BOOL task_rsp_encode(TASK_RSP *task_rsp)
{
    ...
    if(e_dbg_void != func_addr_node->func_ret_type)
    {
        ...
        if(EC_TRUE == TYPE_CONV_ITEM_VAR_POINTER_FLAG(type_conv_item) && 0 != task_rsp_func->func_ret_val)
        {
            ...
            free_static_mem(MD_TASK, CMPI_ANY_MODI, TYPE_CONV_ITEM_VAR_MM_TYPE(type_conv_item), (void *)task_rsp_func->func_ret_val, LOC_TASK_0018);/*clean up*/
            task_rsp_func->func_ret_val = 0;
        }
    }
...
{
    TASK_NODE_CMUTEX_LOCK(TASK_REQ_NODE(task_req), LOC_TASK_0019);

    TASK_NODE_STATUS(TASK_REQ_NODE(task_req)) = TASK_REQ_DISCARD;

    TASK_MGR_COUNTER_INC_BY_TASK_REQ(TASK_REQ_MGR(task_req), TASK_MGR_COUNTER_TASK_REQ_DISCARD, task_req, LOC_TASK_0020);

    TASK_NODE_CMUTEX_UNLOCK(TASK_REQ_NODE(task_req), LOC_TASK_0021);
...
{
    TASK_MGR_COUNTER_DEC_BY_TASK_RSP(TASK_RSP_MGR(task_rsp), TASK_MGR_COUNTER_TASK_RSP_RESERVD, task_rsp, LOC_TASK_0022);
    return;
}
...
EC_BOOL task_rsp_reserve(const TASK_BRD *task_brd, TASK_RSP *task_rsp, TASK_MGR **task_mgr_ret, TASK_REQ **task_req_ret)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0023);/*1st lock*/
    ...
    CLIST_LOCK(task_queue, LOC_TASK_0025);/*2nd lock*/
    ...
    if(TASK_MGR_SEQNO(task_mgr) != TASK_REQ_SEQNO(task_req) || task_mgr != TASK_REQ_MGR(task_req))
    {
        ...
        CLIST_UNLOCK(task_queue, LOC_TASK_0028);
        CLIST_UNLOCK(task_mgr_list, LOC_TASK_0029);
        return (EC_FALSE);
    }
    ...
    *task_req_ret = task_req;
    ...
    CLIST_UNLOCK(task_queue, LOC_TASK_0030);
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0031);
    return (EC_TRUE);
}
...
EC_BOOL task_rsp_decode(const UINT32 recv_comm, TASK_BRD *task_brd, TASK_RSP *task_rsp, TASK_MGR **task_mgr_ret, UINT32 *ret_val_check_succ_flag)
{
    ...
    TASK_MGR_COUNTER_INC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_RESERVD, task_rsp, LOC_TASK_0032);

    CROUTINE_CLEANUP_PUSH(task_rsp_decode_thread_cancel, task_rsp);

    //TASK_MGR_CMUTEX_LOCK(task_mgr, LOC_TASK_0033);
    ...
    {
        (*task_mgr_ret) = task_mgr;
        //TASK_MGR_CMUTEX_UNLOCK(task_mgr, LOC_TASK_0034);/*unlock*/
        TASK_MGR_COUNTER_DEC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_RESERVD, task_rsp, LOC_TASK_0035);
        return (EC_FALSE);
    }
    //TASK_MGR_CMUTEX_UNLOCK(task_mgr, LOC_TASK_0036);/*unlock*/

    TASK_NODE_CMUTEX_LOCK(TASK_REQ_NODE(task_req), LOC_TASK_0037);
    if( TASK_REQ_DISCARD == TASK_NODE_STATUS(TASK_REQ_NODE(task_req))
      || TASK_REQ_TIMEOUT == TASK_NODE_STATUS(TASK_REQ_NODE(task_req))
    )
    {
        (*task_mgr_ret) = task_mgr;
        TASK_NODE_CMUTEX_UNLOCK(TASK_REQ_NODE(task_req), LOC_TASK_0038);
        TASK_MGR_COUNTER_DEC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_RESERVD, task_rsp, LOC_TASK_0039);
        return (EC_FALSE);
    }
    else
    {
    ...
    TASK_NODE_CMUTEX_UNLOCK(TASK_REQ_NODE(task_req), LOC_TASK_0040);
    ...
    TASK_MGR_COUNTER_DEC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_RESERVD, task_rsp, LOC_TASK_0041);
...
{
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "isend  rsp: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
...
EC_BOOL task_rsp_md_mod_mgr_get(TASK_BRD *task_brd, TASK_RSP *task_rsp, MOD_MGR **mod_mgr)
{
    ...
    md_mod_mgr = (MOD_MGR *)(func_addr_mgr->md_fget_mod_mgr)(md_id);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_rsp_md_mod_mgr_get: md_type %ld, md_id %ld, md_mod_mgr %lx\n", md_type, md_id, md_mod_mgr);
...
EC_BOOL task_fwd_isend(TASK_BRD *task_brd, TASK_FWD *task_fwd)
{
    ...
    {
    case TAG_TASK_REQ:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "isend  fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
    ...
    case TAG_TASK_FWD:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "isend  fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno fwd.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                    TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                    TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                    TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
    ...
    default:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "isend  fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno undef.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                    TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                    TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                    TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
...
EC_BOOL task_fwd_direct(TASK_BRD *task_brd, TASK_FWD *task_fwd)
{
    ...
    {
    case TAG_TASK_REQ:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "direct fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                    TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                    TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                    TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
    ...
    case TAG_TASK_FWD:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "direct fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno fwd.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                    TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                    TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                    TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
    ...
    default:
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "direct fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno undef.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                    TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                    TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                    TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
...
EC_BOOL task_req_cancel(TASK_REQ *task_req)
{
    ...
    //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_req_cancel: mod_id %ld, md %lx\n", mod_id, cbc_md_get(mod_type, mod_id));
    ...
    if(EC_TRUE == ret)
    {
        dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "cancel req: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                        TASK_REQ_SEND_TCID_STR(task_req), TASK_REQ_SEND_COMM(task_req), TASK_REQ_SEND_RANK(task_req), TASK_REQ_SEND_MODI(task_req),
                        TASK_REQ_RECV_TCID_STR(task_req), TASK_REQ_RECV_COMM(task_req), TASK_REQ_RECV_RANK(task_req), TASK_REQ_RECV_MODI(task_req),
                        TASK_REQ_PRIO(task_req), TASK_REQ_TYPE(task_req),
                        TASK_REQ_TAG(task_req), TASK_REQ_LDB_CHOICE(task_req),
...
EC_BOOL task_req_local_handle(TASK_REQ *task_req)
{
    ...
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "direct req: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_REQ_SEND_TCID_STR(task_req), TASK_REQ_SEND_COMM(task_req), TASK_REQ_SEND_RANK(task_req), TASK_REQ_SEND_MODI(task_req),
                    TASK_REQ_RECV_TCID_STR(task_req),TASK_REQ_RECV_COMM(task_req), TASK_REQ_RECV_RANK(task_req), TASK_REQ_RECV_MODI(task_req),
                    TASK_REQ_PRIO(task_req), TASK_REQ_TYPE(task_req),
                    TASK_REQ_TAG(task_req),  TASK_REQ_LDB_CHOICE(task_req),
    ...
    if(TASK_ACT_TYPE != TASK_REQ_TYPE(task_req))
    {
        ...
        //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_req_handle: mod_id %ld, md %lx\n", mod_id, cbc_md_get(mod_type, mod_id));
...
void task_req_local_handle_thread(TASK_BRD *task_brd, TASK_REQ *task_req)
{
    ...
    if(NULL_PTR == func_addr_node)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_req_local_handle_thread: func_addr_node is null\n");
        return;
    }

    //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_req_local_handle_thread: when enter  , task_req %lx, func_ret_val = %lx\n", task_req, task_req_func->func_ret_val);
    ...
    if(EC_FALSE == ret)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_req_local_handle_thread: task_req_local_handle failed\n");
        return;
    }

    //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_req_local_handle_thread: after handle, task_req %lx, func_ret_val = %lx\n", task_req, task_req_func->func_ret_val);
    ...
    if(e_dbg_void != func_addr_node->func_ret_type)
    {
        ...
        *(UINT32 *)task_req_retval = (task_req_func->func_ret_val);                
        ...
        if( NULL_PTR == type_conv_item )
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_req_local_handle_thread: ret type %ld conv item is not defined\n",
                               func_addr_node->func_ret_type);
            exit(0);/*coding bug, user should fix it*/
        }
        ...
        if(NULL_PTR != task_mgr && NULL_PTR != TASK_MGR_RETV_CHECKER(task_mgr))
        {
            ...
            else
            {
                ...
                {
                    dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_req_local_handle_thread: type %ld has unknow sizeof %ld\n",
                                        TYPE_CONV_ITEM_VAR_DBG_TYPE(type_conv_item), TYPE_CONV_ITEM_VAR_SIZEOF(type_conv_item));
                }
            }
        }
    ...
    {
        TASK_MGR_COUNTER_INC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_RSP_IS_SUCC, task_req, LOC_TASK_0042);
    }
    else
    {
        TASK_MGR_COUNTER_INC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_RSP_IS_FAIL, task_req, LOC_TASK_0043);
    }
...
TASK_RSP * task_req_handle(TASK_REQ *task_req)
{
    ...
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "handle req: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_REQ_SEND_TCID_STR(task_req), TASK_REQ_SEND_COMM(task_req), TASK_REQ_SEND_RANK(task_req), TASK_REQ_SEND_MODI(task_req),
                    TASK_REQ_RECV_TCID_STR(task_req),TASK_REQ_RECV_COMM(task_req), TASK_REQ_RECV_RANK(task_req), TASK_REQ_RECV_MODI(task_req),
                    TASK_REQ_PRIO(task_req), TASK_REQ_TYPE(task_req),
                    TASK_REQ_TAG(task_req),  TASK_REQ_LDB_CHOICE(task_req),
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_handle: task_req is ===========================================\n");
    //task_req_print(LOGSTDOUT, task_req);

    //func_addr_node = TASK_REQ_FUNC_ADDR_NODE(task_req);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_handle:: func_addr_node is ===========================================\n");
    //func_addr_node_print(LOGSTDOUT, func_addr_node);
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_req_handle: task_req_func func_ret_val = %ld\n", task_req_func->func_ret_val);
...
EC_BOOL task_context_handle(TASK_BRD *task_brd, const TASK_RSP *task_rsp_ret)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_context_handle: TASK_RSP_FUNC_RET is ERR_MODULE_ID\n");
        return (EC_FALSE);
    }
    ...
    if(TASK_ACT_TYPE == TASK_RSP_TYPE(task_rsp_ret) && ERR_MODULE_ID != TASK_RSP_FUNC_RET(task_rsp_ret))
    {
        ...
        task_rsp = task_rsp_new(0, LOC_TASK_0045);
    ...
    if(TASK_DEA_TYPE == TASK_RSP_TYPE(task_rsp_ret))
    {
        ...
        CLIST_LOCK(TASK_BRD_CONTEXT_LIST(task_brd), LOC_TASK_0046);
        ...
        CLIST_UNLOCK(TASK_BRD_CONTEXT_LIST(task_brd), LOC_TASK_0048);
        return (EC_TRUE);
    }
    return (EC_TRUE);
}
...
EC_BOOL task_context_discard_from(TASK_BRD *task_brd, const UINT32 broken_tcid)
{
    ...
    CLIST_LOCK(TASK_BRD_CONTEXT_LIST(task_brd), LOC_TASK_0049);
    CLIST_LOOP_NEXT(TASK_BRD_CONTEXT_LIST(task_brd), clist_data)
    {
        TASK_CONTEXT *task_context;
        TASK_RSP     *task_rsp;
    ...
    CLIST_UNLOCK(TASK_BRD_CONTEXT_LIST(task_brd), LOC_TASK_0050);
    return (EC_TRUE);
}
...
{
    clist_init(task_queue, MM_IGNORE, LOC_TASK_0051);
    return (EC_TRUE);
}
...
EC_BOOL task_queue_add_node(CLIST *task_queue, const TASK_NODE *task_node)
{
    ...
    switch(TASK_ANY_PRIO(TASK_NODE_ANY(task_node)))
    {
        ...
        case TASK_PRIO_HIGH:
        {
            ...
            CLIST_LOCK(task_queue, LOC_TASK_0052);
            ...
            CLIST_UNLOCK(task_queue, LOC_TASK_0054);
...
TASK_RANK_NODE * task_rank_node_new()
{
    ...
    alloc_static_mem(MD_TASK, 0, MM_TASK_RANK_NODE, &task_rank_node, LOC_TASK_0055);
...
{
    TASK_RANK_NODE_CMUTEX_INIT(task_rank_node, LOC_TASK_0056);
...
EC_BOOL task_rank_node_clean(TASK_RANK_NODE *task_rank_node)
{
    ...
    TASK_RANK_NODE_CMUTEX_CLEAN(task_rank_node, LOC_TASK_0057);
    return (EC_TRUE);
}
...
EC_BOOL task_rank_node_free(TASK_RANK_NODE *task_rank_node)
{
    ...
    free_static_mem(MD_TASK, 0, MM_TASK_RANK_NODE, task_rank_node, LOC_TASK_0058);
    return (EC_TRUE);
}
...
{
    TASK_RANK_NODE_LOCK(task_rank_node, LOC_TASK_0061);
    ...
    TASK_RANK_NODE_UNLOCK(task_rank_node, LOC_TASK_0063);
    return (EC_FALSE);
}

EC_BOOL task_rank_node_disable(TASK_RANK_NODE *task_rank_node)
{
    TASK_RANK_NODE_LOCK(task_rank_node, LOC_TASK_0064);
    TASK_RANK_NODE_LIGHT(task_rank_node) = TASK_RANK_NODE_RED_LIGHT;
    TASK_RANK_NODE_UNLOCK(task_rank_node, LOC_TASK_0065);
    return (EC_TRUE);
}
...
CVECTOR * task_rank_tbl_new(const UINT32 size)
{
    ...
    task_rank_tbl = cvector_new(size, MM_TASK_RANK_NODE, LOC_TASK_0066);
...
{
    cvector_init(task_rank_tbl, 0, MM_TASK_RANK_NODE, CVECTOR_LOCK_ENABLE, LOC_TASK_0067);
    return (EC_TRUE);
}
*/
EC_BOOL task_rank_tbl_clean(CVECTOR *task_rank_tbl)
{
    cvector_clean(task_rank_tbl, (CVECTOR_DATA_CLEANER)task_rank_node_free, LOC_TASK_0068);
    return (EC_TRUE);
}
...
EC_BOOL task_rank_tbl_free(CVECTOR *task_rank_tbl)
{
    ...
    cvector_free(task_rank_tbl, LOC_TASK_0069);
    return (EC_TRUE);
}
...
EC_BOOL task_queue_discard_from(TASK_BRD *task_brd, CLIST *task_queue, const UINT32 tag, const UINT32 tcid)
{
    ...
    CLIST_LOCK(task_queue, LOC_TASK_0070);
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        TASK_NODE  *task_node;
        TASK_ANY   *task_any;
...
******************************************************************************************************************************/
    ...
    CLIST_UNLOCK(task_queue, LOC_TASK_0071);
    return (EC_TRUE);
}
...
EC_BOOL task_queue_discard_to(TASK_BRD *task_brd, CLIST *task_queue, const UINT32 tag, const UINT32 tcid)
{
    ...
    CLIST_LOCK(task_queue, LOC_TASK_0072);
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        TASK_NODE  *task_node;
        TASK_ANY   *task_any;
    ...
    CLIST_UNLOCK(task_queue, LOC_TASK_0073);
    return (EC_TRUE);
}
...
EC_BOOL task_mgr_list_handle_broken_taskcomm(TASK_BRD *task_brd, const UINT32 tcid)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0074);
    ...
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0075);
...
void task_brd_task_mgr_list_print(LOG *log, const TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0076);
    CLIST_LOOP_NEXT(task_mgr_list, clist_data)
    {
        TASK_MGR *task_mgr;
        task_mgr = (TASK_MGR *)CLIST_DATA_DATA(clist_data);
    ...
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0077);
    return;
}
...
EC_BOOL task_brd_mod_mgr_list_excl(TASK_BRD *task_brd, const UINT32 tcid)
{
    ...
    CLIST_LOCK(mod_mgr_list, LOC_TASK_0078);
    ...
    CLIST_UNLOCK(mod_mgr_list, LOC_TASK_0079);
    return (EC_TRUE);
}
...
EC_BOOL task_mgr_init(const UINT32 seqno, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num, const MOD_MGR *mod_mgr, TASK_MGR *task_mgr)
{
    ...
    //sys_log(LOGSTDOUT, "[DEBUG] [tid %d] task_mgr_init: init task_mgr %p\n", CTHREAD_GET_TID(), task_mgr);
    ...
    TASK_MGR_CCOND_INIT(task_mgr, LOC_TASK_0080);
    TASK_MGR_CRWLOCK_INIT(task_mgr, LOC_TASK_0081);

    //sys_log(LOGSTDOUT, "[DEBUG] [tid %d] task_mgr_init: task_mgr %p, ccond %p, ->var.__data.__nwaiters %d\n", CTHREAD_GET_TID(), task_mgr, (TASK_MGR_CROUTINE_COND(task_mgr)), (TASK_MGR_CROUTINE_COND(task_mgr))->var.__data.__nwaiters);
    return (EC_TRUE);
}
...
{
    //sys_log(LOGSTDOUT, "[DEBUG] [tid %d] task_mgr_clean: clean task_mgr %p\n", CTHREAD_GET_TID(), task_mgr);
    TASK_MGR_CRWLOCK_WRLOCK(task_mgr, LOC_TASK_0082);
    ...
    TASK_MGR_CRWLOCK_UNLOCK(task_mgr, LOC_TASK_0083);

    //TASK_MGR_CCOND_RELEASE_ALL(task_mgr, LOC_TASK_0084);
    TASK_MGR_CCOND_RELEASE(task_mgr, LOC_TASK_0085);
    TASK_MGR_CCOND_CLEAN(task_mgr, LOC_TASK_0086);
    TASK_MGR_CRWLOCK_CLEAN(task_mgr, LOC_TASK_0087);
#if 0
    sys_log(LOGSTDOUT, "[DEBUG] [tid %d] task_mgr_clean: task_mgr %p, ccond %p, ->var.__data.__nwaiters %d\n", 
                        CTHREAD_GET_TID(), 
...
EC_BOOL task_mgr_free(TASK_MGR *task_mgr)
{
    ...
    //sys_log(LOGSTDOUT, "[DEBUG] [tid %d] task_mgr_free: task_mgr %p, ccond %p, ->var.__data.__nwaiters %d\n", CTHREAD_GET_TID(), task_mgr, (TASK_MGR_CROUTINE_COND(task_mgr)), (TASK_MGR_CROUTINE_COND(task_mgr))->var.__data.__nwaiters);
    free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_TASK_MGR, task_mgr, LOC_TASK_0088);
...
EC_BOOL task_mgr_longjmp(TASK_MGR *task_mgr)
{
    ...
    /*then, jump back*/
    //longjmp(env, 1);
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "-----------longjmp: %lx\n", task_mgr);
...
TASK_REQ * task_mgr_search_task_req_by_recver(const TASK_MGR *task_mgr, const UINT32 seqno, const UINT32 subseqno, const MOD_NODE *recv_mod_node)
{
    ...
    CLIST_LOCK(task_queue, LOC_TASK_0092);
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        TASK_NODE  *task_node;
        TASK_REQ   *task_req;
        ...
        {
            CLIST_UNLOCK(task_queue, LOC_TASK_0093);
            return (task_req);
        }
    }
    CLIST_UNLOCK(task_queue, LOC_TASK_0094);
    return (NULL_PTR);
}
...
TASK_CONTEXT * task_context_new()
{
    ...
    alloc_static_mem(MD_TASK, 0, MM_TASK_CONTEXT, &task_context, LOC_TASK_0095);
...
EC_BOOL task_context_free(TASK_CONTEXT *task_context)
{
    if(NULL_PTR != task_context)
    {
        ...
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_TASK_CONTEXT, task_context, LOC_TASK_0096);
    }
    return (EC_TRUE);
}
...
{
    alloc_static_mem(MD_TASK, 0, MM_TASK_REPORT_NODE, task_report_node, LOC_TASK_0097);
...
EC_BOOL task_report_node_free(TASK_REPORT_NODE *task_report_node)
{
    if(NULL_PTR != task_report_node)
    {
        ...
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_TASK_REPORT_NODE, task_report_node, LOC_TASK_0098);
    }
    return (EC_TRUE);
}
...
EC_BOOL task_brd_md_node_tbl_init(TASK_BRD *task_brd)
{
    ...
    cvector_init(TASK_BRD_MD_NODE_TBL(task_brd), MD_END, MM_FUNC_ADDR_MGR, CVECTOR_LOCK_ENABLE, LOC_TASK_0099);
...
{
    cvector_init(TASK_BRD_BROKEN_TCID_TBL(task_brd), 0, MM_UINT32, CVECTOR_LOCK_ENABLE, LOC_TASK_0100);
    return (EC_TRUE);
}

EC_BOOL task_brd_mod_mgr_list_init(TASK_BRD *task_brd)
{
    clist_init(TASK_BRD_MOD_MGR_LIST(task_brd), MM_IGNORE, LOC_TASK_0101);
    return (EC_TRUE);
}

EC_BOOL task_brd_context_list_init(TASK_BRD *task_brd)
{
    clist_init(TASK_BRD_CONTEXT_LIST(task_brd), MM_IGNORE, LOC_TASK_0102);
    return (EC_TRUE);
}

EC_BOOL task_brd_report_list_init(TASK_BRD *task_brd)
{
    clist_init(TASK_BRD_REPORT_LIST(task_brd), MM_IGNORE, LOC_TASK_0103);
    return (EC_TRUE);
}

void task_brd_report_list_add(TASK_BRD *task_brd, const TASK_MGR *task_mgr)
{
    CLIST_LOCK(TASK_BRD_REPORT_LIST(task_brd), LOC_TASK_0104);
    ...
    CLIST_UNLOCK(TASK_BRD_REPORT_LIST(task_brd), LOC_TASK_0105);
    return;
}
...
void task_brd_report_list_dump(TASK_BRD *task_brd, const UINT32 num, CVECTOR *task_report_vec_des)
{
    ...
    CLIST_LOCK(TASK_BRD_REPORT_LIST(task_brd), LOC_TASK_0106);
    ...
    CLIST_UNLOCK(TASK_BRD_REPORT_LIST(task_brd), LOC_TASK_0107);
...
{
    g_task_brd = (TASK_BRD *)SAFE_MALLOC(sizeof(TASK_BRD), LOC_TASK_0108);
                        ...
                        CSTRING *log_path_cstr)
{
    ...
    TASK_BRD_RECV_TASK_MGR_LIST_INIT(task_brd, LOC_TASK_0109);
    TASK_BRD_ARGING_TASK_MGR_LIST_INIT(task_brd, LOC_TASK_0110);
    TASK_BRD_TASK_MGR_STACK_INIT(task_brd, LOC_TASK_0111);
    ...
    TASK_BRD_FWD_CCOND_INIT(task_brd, LOC_TASK_0112);
    ...
    TASK_BRD_SEQNO_CMUTEX_INT(task_brd, LOC_TASK_0113);
...
EC_BOOL task_brd_wait_sys_config(TASK_BRD *task_brd, const UINT32 udp_mcast_ipaddr, const UINT32 udp_mcast_port)
{
    ...
    if(EC_FALSE == csocket_udp_mcast_recvfrom(sockfd, udp_mcast_ipaddr, udp_mcast_port, cbytes_buf(src_cbytes), cbytes_len(src_cbytes), &src_len))
    {
        ...
        csocket_stop_udp_mcast_recver(sockfd, udp_mcast_ipaddr);
        cbytes_free(src_cbytes, LOC_TASK_0115);
        cbytes_free(des_cbytes, LOC_TASK_0116);
        return (EC_FALSE);
    }
    ...
    if(Z_OK != uncompress(cbytes_buf(des_cbytes), &des_len, cbytes_buf(src_cbytes), cbytes_len(src_cbytes)))
    {
        ...
        cbytes_free(src_cbytes, LOC_TASK_0117);
        cbytes_free(des_cbytes, LOC_TASK_0118);
        return (EC_FALSE);
    }
    cbytes_free(src_cbytes, LOC_TASK_0119);/*clean up src bytes cache*/
    ...
    cbytes_free(des_cbytes, LOC_TASK_0121);
...
EC_BOOL task_brd_mcast_config(TASK_BRD *task_brd)
{
    ...
    if(Z_OK != compress(cbytes_buf(des_cbytes), &des_len, cbytes_buf(src_cbytes), cbytes_len(src_cbytes)))
    {
        ...
        cbytes_free(src_cbytes, LOC_TASK_0124);
        cbytes_free(des_cbytes, LOC_TASK_0125);
        return (EC_FALSE);
    }

    src_len = cbytes_len(src_cbytes);/*save length info before free*/
    cbytes_free(src_cbytes, LOC_TASK_0126);
    cbytes_resize(des_cbytes, des_len);
    ...
    cbytes_free(des_cbytes, LOC_TASK_0128);
...
EC_BOOL task_brd_start_ganglia(TASK_BRD *task_brd)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_ganglia_udp: start ganglia reporting to %s:%ld failed\n",
    ...
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_start_ganglia_udp: start ganglia reporting to %s:%ld\n",
                        GANGLIA_CFG_IPADDR_STR(ganglia_cfg), GANGLIA_CFG_PORT(ganglia_cfg));
    return (EC_TRUE);
}
...
EC_BOOL task_brd_collect_netcards(TASK_BRD *task_brd)
{
    if(NULL_PTR == TASK_BRD_NETCARDS(task_brd))
    {
        ...
        cnetcard_set = cset_new(MM_IGNORE, LOC_TASK_0129);
                                    ...
                                    EC_BOOL  *daemon_flag)
{
    ...
    for(idx = 0; idx < argc; idx ++)
    {
        ...
        {
            (*sys_cfg_xml_fname_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0130);
            continue;
        }

        if(0 == strcasecmp(argv[idx], "-bconfig") && idx + 1 < argc)
        {
            (*basic_cfg_xml_fname_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0131);
            continue;
        }

        if(0 == strcasecmp(argv[idx], "-eth") && idx + 1 < argc)
        {
            (*bcast_dhcp_netcard_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0132);
            continue;
        }

        if(0 == strcasecmp(argv[idx], "-logp") && idx + 1 < argc)
        {
            (*log_path_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0133);
            continue;
        }

        if(0 == strcasecmp(argv[idx], "-pidfile") && idx + 1 < argc)/*optional*/
        {
            (*pid_path_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0134);
            continue;
        }
        ...
        {
            (*console_path_cstr) = cstring_new((UINT8 *)argv[idx + 1], LOC_TASK_0135);
            continue;
        }
    }
#if 1
    if(NULL_PTR == (*sys_cfg_xml_fname_cstr))
    {
        ...
        (*sys_cfg_xml_fname_cstr) = cstring_new((UINT8 *)"config.xml", LOC_TASK_0136);
        if(NULL_PTR == (*sys_cfg_xml_fname_cstr))
        {
    ...
    if(NULL_PTR == (*basic_cfg_xml_fname_cstr))
    {
        ...
        (*basic_cfg_xml_fname_cstr) = cstring_new((UINT8 *)"basic.xml", LOC_TASK_0137);
        if(NULL_PTR == (*basic_cfg_xml_fname_cstr))
        {
    ...
    {
        /*set default basic xml info*/
        (*bcast_dhcp_netcard_cstr) = cstring_new((UINT8 *)"eth0", LOC_TASK_0138);
        if(NULL_PTR == (*bcast_dhcp_netcard_cstr))
        {
    ...
    if(NULL_PTR == (*log_path_cstr))
    {
        ...
        (*log_path_cstr) = cstring_new((UINT8 *)"./", LOC_TASK_0139);
        if(NULL_PTR == (*log_path_cstr))
        {
...
EC_BOOL task_brd_register_one(TASK_BRD *task_brd, const UINT32 remote_tcid, const UINT32 remote_srv_ipaddr, const UINT32 remote_srv_port, const UINT32 conn_num)
{
    ...
    for(csocket_cnode_idx = conn_count; csocket_cnode_idx < conn_num; csocket_cnode_idx ++)
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_register_one: failed register to remote tasks tcid %s srvipaddr %s srvport %ld\n",
                                c_word_to_ipv4(remote_tcid), c_word_to_ipv4(remote_srv_ipaddr),remote_srv_port);
            return (EC_FALSE);
        }
    }
...
EC_BOOL task_brd_register_node(TASK_BRD *task_brd, const UINT32 tcid)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_register_node: register to remote tasks tcid %s: "
...
EC_BOOL task_brd_register_all(TASK_BRD *task_brd, CLUSTER_CFG *cluster_cfg)
{
    ...
    CVECTOR_LOCK(cluster_nodes, LOC_TASK_0140);
    ...
    CVECTOR_UNLOCK(cluster_nodes, LOC_TASK_0141);
...
EC_BOOL task_brd_register_role_str(TASK_BRD *task_brd, CLUSTER_CFG *cluster_cfg, const char *role_str)
{
    ...
    CVECTOR_LOCK(cluster_nodes, LOC_TASK_0142);
    for(pos = 0; pos < cvector_size(cluster_nodes); pos ++)
    {
        CLUSTER_NODE_CFG *cluster_node_cfg;
    ...
    CVECTOR_UNLOCK(cluster_nodes, LOC_TASK_0143);
...
EC_BOOL task_brd_register_role_str_and_group_cstr(TASK_BRD *task_brd, CLUSTER_CFG *cluster_cfg, const char *role_str, CSTRING *group_cstr)
{
    ...
    CVECTOR_LOCK(cluster_nodes, LOC_TASK_0144);
    for(pos = 0; pos < cvector_size(cluster_nodes); pos ++)
    {
        CLUSTER_NODE_CFG *cluster_node_cfg;
    ...
    CVECTOR_UNLOCK(cluster_nodes, LOC_TASK_0145);
...
EC_BOOL task_brd_register_cluster(TASK_BRD *task_brd)
{
    ...
    while(EC_FALSE == tasks_monitor_is_empty(TASKS_CFG_MONITOR(tasks_cfg)))
    {
        dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_brd_register_cluster: sleep and wait socket connection ready ...\n");
        c_sleep(1, LOC_TASK_0146);
    }
...
LOG * task_brd_default_init(int argc, char **argv)
{
    ...
    log_file_name = cstring_new(NULL_PTR, LOC_TASK_0148);
    ...
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "loaded sysconfig:\n");
    sys_cfg_print_xml(LOGSTDOUT, TASK_BRD_SYS_CFG(task_brd), 0);

    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "current paraconfig:\n");
    cparacfg_print_xml(log, TASK_BRD_CPARACFG(task_brd), 0);

    if(SWITCH_ON == LICENSE_SWITCH && EC_FALSE == lic_check())
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error: invalid license\n");
        dbg_log(SEC_0015_TASK, 0)(LOGCONSOLE, "error: invalid license\n");
    ...
    TASK_BRD_FWD_CCOND_RESERVE(task_brd, 1, LOC_TASK_0149);
    ...
    do_slave_enhanced(task_brd);/*here enter main loop*/
#endif/*(SWITCH_ON == CROUTINE_SUPPORT_COROUTINE_SWITCH)*/
    ...
    if (CMPI_FWD_RANK == TASK_BRD_RANK(task_brd))
    {
        ...
        TASK_BRD_FWD_CCOND_RELEASE_ALL(task_brd, LOC_TASK_0150);
    }
    ...
    if(CMPI_FWD_RANK != TASK_BRD_RANK(task_brd))
    {
        ...
        TASK_BRD_FWD_CCOND_RELEASE_ALL(task_brd, LOC_TASK_0151);
    }
...
EC_BOOL task_brd_mod_mgr_add(TASK_BRD *task_brd, MOD_MGR *mod_mgr)
{
    ...
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_mod_mgr_add: add mod_mgr %lx to board\n", mod_mgr);
    return (EC_TRUE);
}
...
EC_BOOL task_brd_mod_mgr_rmv(TASK_BRD *task_brd, MOD_MGR *mod_mgr)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_mod_mgr_rmv: not find mod_mgr %lx in task_brd\n", mod_mgr);
        return (EC_FALSE);
    }
    //dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_mod_mgr_rmv: remove mod_mgr %lx from board\n", mod_mgr);
    return (EC_TRUE);
}

UINT32 task_brd_seqno_gen(TASK_BRD *task_brd, UINT32 *seqno_new)
{
    TASK_BRD_SEQNO_CMUTEX_LOCK(task_brd, LOC_TASK_0155);
    (*seqno_new) = ++ TASK_BRD_SEQNO(task_brd);
    TASK_BRD_SEQNO_CMUTEX_UNLOCK(task_brd, LOC_TASK_0156);
    return (0);
}
...
*/
    ...
    cvector_clean(TASK_BRD_MD_NODE_TBL(task_brd), NULL_PTR, LOC_TASK_0157);
    ...
    cvector_clean(TASK_BRD_BROKEN_TCID_TBL(task_brd), NULL_PTR, LOC_TASK_0158);
...
EC_BOOL task_brd_free(TASK_BRD *task_brd)
{
    if(NULL_PTR != task_brd)
    {
        ...
        SAFE_FREE(task_brd, LOC_TASK_0159);
    }
    return (EC_TRUE);
}
...
EC_BOOL task_brd_cbtimer_register(TASK_BRD *task_brd, const UINT32 expire_nsec, const UINT32 timeout_nsec, const UINT32 timeout_func_id, ...)
{
    ...
    CBTIMER_NODE_NAME(cbtimer_node) = cstring_new((UINT8 *)func_addr_node->func_name, LOC_TASK_0160);
                                     ...
                                     const UINT32 timeout_nsec, FUNC_ADDR_NODE *task_brd_timeout_func_addr_node)
{
    ...
    CBTIMER_NODE_NAME(cbtimer_node) = cstring_new(name, LOC_TASK_0161);
...
EC_BOOL task_brd_task_mgr_match(const TASK_BRD *task_brd, const TASK_RSP *task_rsp, TASK_MGR **task_mgr_ret)
{
    ...
    CLIST_LOCK(task_mgr_list, LOC_TASK_0163);
    CLIST_LOOP_NEXT(task_mgr_list, clist_data)
    {
        TASK_MGR *task_mgr;
        ...
        {
            *task_mgr_ret = task_mgr;
            CLIST_UNLOCK(task_mgr_list, LOC_TASK_0164);
            return (EC_TRUE);
        }
    }
    CLIST_UNLOCK(task_mgr_list, LOC_TASK_0165);
    return (EC_FALSE);
}
...
EC_BOOL task_brd_commit_req(TASK_BRD *task_brd, TASK_REQ *task_req)
{
    ...
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "commit req: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_REQ_SEND_TCID_STR(task_req), TASK_REQ_SEND_COMM(task_req), TASK_REQ_SEND_RANK(task_req), TASK_REQ_SEND_MODI(task_req),
                    TASK_REQ_RECV_TCID_STR(task_req), TASK_REQ_RECV_COMM(task_req), TASK_REQ_RECV_RANK(task_req), TASK_REQ_RECV_MODI(task_req),
                    TASK_REQ_PRIO(task_req), TASK_REQ_TYPE(task_req),
                    TASK_REQ_TAG(task_req), TASK_REQ_LDB_CHOICE(task_req),
...
{
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "commit rsp: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                    TASK_RSP_SEND_TCID_STR(task_rsp), TASK_RSP_SEND_COMM(task_rsp), TASK_RSP_SEND_RANK(task_rsp), TASK_RSP_SEND_MODI(task_rsp),
                    TASK_RSP_RECV_TCID_STR(task_rsp), TASK_RSP_RECV_COMM(task_rsp), TASK_RSP_RECV_RANK(task_rsp), TASK_RSP_RECV_MODI(task_rsp),
                    TASK_RSP_PRIO(task_rsp), TASK_RSP_TYPE(task_rsp),
                    TASK_RSP_TAG(task_rsp), TASK_RSP_LDB_CHOICE(task_rsp),
...
EC_BOOL task_brd_commit_fwd(TASK_BRD *task_brd, TASK_FWD *task_fwd)
{
    ...
    {
        case TAG_TASK_REQ:
        dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "commit fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno %lx.%lx.%lx, subseqno %ld, func id %lx\n",
                        TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                        TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                        TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                        TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
        ...
        case TAG_TASK_FWD:
        dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "commit fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno fwd.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                        TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                        TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                        TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                        TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
        ...
        default:
        dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "commit fwd: from (tcid %s,comm %ld,rank %ld,modi %ld) to (tcid %s,comm %ld,rank %ld,modi %ld) with priority %ld, type %ld, tag %ld, ldb %ld, seqno undef.%lx.%lx.%lx, subseqno %ld, func id %lx\n",
                        TASK_FWD_SEND_TCID_STR(task_fwd), TASK_FWD_SEND_COMM(task_fwd), TASK_FWD_SEND_RANK(task_fwd), TASK_FWD_SEND_MODI(task_fwd),
                        TASK_FWD_RECV_TCID_STR(task_fwd), TASK_FWD_RECV_COMM(task_fwd), TASK_FWD_RECV_RANK(task_fwd), TASK_FWD_RECV_MODI(task_fwd),
                        TASK_FWD_PRIO(task_fwd), TASK_FWD_TYPE(task_fwd),
                        TASK_FWD_TAG(task_fwd), TASK_FWD_LDB_CHOICE(task_fwd),
...
EC_BOOL task_brd_to_send_queue_handle(TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(TASK_BRD_QUEUE(task_brd, TASK_TO_SEND_QUEUE), LOC_TASK_0166);
    ...
    CLIST_UNLOCK(TASK_BRD_QUEUE(task_brd, TASK_TO_SEND_QUEUE), LOC_TASK_0167);
    return (EC_TRUE);
}
...
EC_BOOL task_brd_sending_queue_handle(TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(TASK_BRD_QUEUE(task_brd, TASK_SENDING_QUEUE), LOC_TASK_0168);
    ...
    CLIST_UNLOCK(TASK_BRD_QUEUE(task_brd, TASK_SENDING_QUEUE), LOC_TASK_0169);
    return (EC_TRUE);
}
...
UINT32 task_rsp_decode_thread(TASK_BRD *task_brd, TASK_NODE *task_node)
{
    ...
    if(EC_FALSE == task_rsp_decode(TASK_BRD_COMM(task_brd), task_brd, task_rsp, &task_mgr, &ret_val_check_succ_flag))
    {
        ...
        {
            TASK_MGR_COUNTER_INC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_IS_FAIL, task_rsp, LOC_TASK_0170);
    ...
    else
    {
        ...
        if(EC_TRUE == ret_val_check_succ_flag)
        {
            TASK_MGR_COUNTER_INC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_IS_SUCC, task_rsp, LOC_TASK_0171);
            task_rsp_succ_dbg_info(task_mgr, task_rsp);
        }
        else
        {
            TASK_MGR_COUNTER_INC_BY_TASK_RSP(task_mgr, TASK_MGR_COUNTER_TASK_RSP_IS_FAIL, task_rsp, LOC_TASK_0172);
...
EC_BOOL task_brd_recving_queue_handle(TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(TASK_BRD_QUEUE(task_brd, TASK_RECVING_QUEUE), LOC_TASK_0173);
    CLIST_LOOP_NEXT(TASK_BRD_QUEUE(task_brd, TASK_RECVING_QUEUE), clist_data)
    {
        ...
        if(NULL_PTR != croutine_node)
        {
            ...
            CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_TASK_0174);
            //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_brd_recving_queue_handle: set cthread_node %lx to busy\n", TASK_REQ_CTHREAD_NODE(task_req));
        }
        else
        {
            break;
    ...
    CLIST_UNLOCK(TASK_BRD_QUEUE(task_brd, TASK_RECVING_QUEUE), LOC_TASK_0175);
    return (EC_TRUE);
}
...
EC_BOOL task_brd_is_recv_queue_handle(TASK_BRD *task_brd)
{
    ...
    CLIST_LOCK(TASK_BRD_QUEUE(task_brd, TASK_IS_RECV_QUEUE), LOC_TASK_0176);
    CLIST_LOOP_NEXT(TASK_BRD_QUEUE(task_brd, TASK_IS_RECV_QUEUE), clist_data)
    {
        ...
        switch(TASK_NODE_TAG(task_node))
        {
            case TAG_TASK_REQ:
            {
                ...
                //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_brd_is_recv_queue_handle: task req %lx, retval %lx\n", task_req, TASK_REQ_FUNC(task_req)->func_ret_val);
                ...
                if(NULL_PTR != TASK_REQ_CTHREAD_NODE(task_req))
                {
                    ...
                    CROUTINE_NODE_COND_RELEASE(TASK_REQ_CTHREAD_NODE(task_req), LOC_TASK_0177);
                }
    ...
    CLIST_UNLOCK(TASK_BRD_QUEUE(task_brd, TASK_IS_RECV_QUEUE), LOC_TASK_0178);
    return (EC_TRUE);
}
    ...
    CLIST_LOOP_NEXT(task_queue, clist_data)
    {
        ...
        if( TASK_REQ_TO_SEND == TASK_NODE_STATUS(task_node) )
        {
            ...
            if(EC_FALSE == task_req_ldb(task_req))
            {
                ...
                TASK_MGR_COUNTER_INC_BY_TASK_REQ(task_mgr, TASK_MGR_COUNTER_TASK_REQ_DISCARD, TASK_NODE_REQ(task_node), LOC_TASK_0181);
            ...
            {
                /*shortcut*/
                //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] task_mgr_send: shortcut task req %lx, retval %lx\n", task_req, TASK_REQ_FUNC(task_req)->func_ret_val);
                TASK_NODE_BUFF_POS(task_node) = 0;
                TASK_NODE_BUFF_LEN(task_node) = 0;
                task_brd_commit_req(task_brd, task_req);
...
EC_BOOL task_brd_start_csolr_srv(TASK_BRD *task_brd, const UINT32 csolr_md_id, const UINT32 csolr_srv_ipaddr, const UINT32 csolr_srv_port)
{
    if(NULL_PTR != TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_csolr_srv: TASK_BRD_CSRV is already working as server at %s:%ld on sockfd %d\n",
                            c_word_to_ipv4(CSRV_IPADDR(TASK_BRD_CSRV(task_brd))),
                            CSRV_PORT(TASK_BRD_CSRV(task_brd)), 
                            CSRV_SOCKFD(TASK_BRD_CSRV(task_brd)));
        return (EC_FALSE);
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(csolr_srv_ipaddr, csolr_srv_port, csolr_md_id, 
                                         (uint32_t)0,
                                         csolr_add_csocket_cnode, csolr_del_csocket_cnode, 
                                         NULL_PTR, NULL_PTR, NULL_PTR, NULL_PTR);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_csolr_srv: start csolr srv failed\n");
        return (EC_FALSE);
    }
    return (EC_TRUE);
}
...
EC_BOOL task_brd_default_start_csolr_srv(const UINT32 csolr_md_id, const UINT32 csolr_srv_ipaddr, const UINT32 csolr_srv_port)
{
    ...
    task_brd = task_brd_default_get();
    return task_brd_start_csolr_srv(task_brd, csolr_md_id, csolr_srv_ipaddr, csolr_srv_port);
}
...
EC_BOOL task_brd_start_crfsngx_srv(TASK_BRD *task_brd, const UINT32 crfs_md_id, const UINT32 crfsngx_srv_ipaddr, const UINT32 crfsngx_srv_port)
{
    if(NULL_PTR != TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_crfsngx_srv: TASK_BRD_CSRV is already working as server at %s:%ld on sockfd %d\n",
                            c_word_to_ipv4(CSRV_IPADDR(TASK_BRD_CSRV(task_brd))),
                            CSRV_PORT(TASK_BRD_CSRV(task_brd)), 
                            CSRV_SOCKFD(TASK_BRD_CSRV(task_brd)));
        return (EC_FALSE);
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(crfsngx_srv_ipaddr, crfsngx_srv_port, crfs_md_id, 
                                        (uint32_t)CRFSHTTP_SOCKET_TIMEOUT_NSEC,
                                        NULL_PTR, NULL_PTR, 
                                        (CSRV_READ_HANDLER)crfsngx_recv_on_csocket_cnode, 
                                        (CSRV_WRITE_HANDLER)crfsngx_send_on_csocket_cnode,
                                        (CSRV_TIMEOUT_HANDLER)csocket_cnode_close,
                                        (CSRV_CLOSE_HANDLER)csocket_cnode_close);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_crfsngx_srv: start crfsngx srv failed\n");
        return (EC_FALSE);
    }

    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_start_crfsngx_srv: start server at port %ld\n", crfsngx_srv_port);    
    return (EC_TRUE);
}
...
EC_BOOL task_brd_default_start_crfsngx_srv(const UINT32 crfs_md_id, const UINT32 crfsngx_srv_ipaddr, const UINT32 crfsngx_srv_port)
{
    ...
    task_brd = task_brd_default_get();
    return task_brd_start_crfsngx_srv(task_brd, crfs_md_id, crfsngx_srv_ipaddr, crfsngx_srv_port);
}
...
EC_BOOL task_brd_start_crfshttp_srv(TASK_BRD *task_brd, const UINT32 crfs_md_id, const UINT32 crfshttp_srv_ipaddr, const UINT32 crfshttp_srv_port)
{
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(crfshttp_srv_ipaddr, crfshttp_srv_port, crfs_md_id, 
                                        (uint32_t)CRFSHTTP_SOCKET_TIMEOUT_NSEC,
                                        NULL_PTR, NULL_PTR, 
                                        (CSRV_READ_HANDLER)crfshttp_recv_on_csocket_cnode, 
                                        (CSRV_WRITE_HANDLER)crfshttp_send_on_csocket_cnode,
                                        (CSRV_TIMEOUT_HANDLER)crfshttp_csocket_cnode_close,
                                        (CSRV_CLOSE_HANDLER)/*crfshttp_csocket_cnode_close*/NULL_PTR);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_crfshttp_srv: start crfshttp srv failed\n");
        return (EC_FALSE);
    }
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_start_crfshttp_srv: start server at port %ld\n", crfshttp_srv_port);    
    return (EC_TRUE);
}
...
EC_BOOL task_brd_default_start_crfshttp_srv(const UINT32 crfs_md_id, const UINT32 crfshttp_srv_ipaddr, const UINT32 crfshttp_srv_port)
{
    ...
    task_brd = task_brd_default_get();
    return task_brd_start_crfshttp_srv(task_brd, crfs_md_id, crfshttp_srv_ipaddr, crfshttp_srv_port);
}
...
EC_BOOL task_brd_start_crfschttp_srv(TASK_BRD *task_brd, const UINT32 crfsc_md_id, const UINT32 crfschttp_srv_ipaddr, const UINT32 crfschttp_srv_port)
{
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(crfschttp_srv_ipaddr, crfschttp_srv_port, crfsc_md_id, 
                                        (uint32_t)CRFSCHTTP_SOCKET_TIMEOUT_NSEC,
                                        NULL_PTR, NULL_PTR, 
                                        (CSRV_READ_HANDLER)crfschttp_recv_on_csocket_cnode_thread, 
                                        /*(CSRV_WRITE_HANDLER)crfschttp_send_on_csocket_cnode_thread*/NULL_PTR,
                                        (CSRV_TIMEOUT_HANDLER)/*crfschttp_csocket_cnode_close*/crfschttp_csocket_cnode_timeout,
                                        (CSRV_CLOSE_HANDLER)/*crfschttp_csocket_cnode_close*/NULL_PTR);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_crfschttp_srv: start crfschttp srv failed\n");
        return (EC_FALSE);
    }
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_start_crfschttp_srv: start server at port %ld\n", crfschttp_srv_port);    
    return (EC_TRUE);
}
...
EC_BOOL task_brd_default_start_crfschttp_srv(const UINT32 crfsc_md_id, const UINT32 crfschttp_srv_ipaddr, const UINT32 crfschttp_srv_port)
{
    ...
    task_brd = task_brd_default_get();
    return task_brd_start_crfschttp_srv(task_brd, crfsc_md_id, crfschttp_srv_ipaddr, crfschttp_srv_port);
}
...
EC_BOOL task_brd_start_cxmpp_srv(TASK_BRD *task_brd, const UINT32 crfs_md_id, const UINT32 cxmpp_srv_ipaddr, const UINT32 cxmpp_srv_port)
{
    ...
    TASK_BRD_CSRV(task_brd) = csrv_start(cxmpp_srv_ipaddr, cxmpp_srv_port, crfs_md_id, 
                                        (uint32_t)CXMPP_SOCKET_TIMEOUT_NSEC,
                                        NULL_PTR, NULL_PTR, 
                                        (CSRV_READ_HANDLER)cxmpp_recv_on_csocket_cnode_thread, 
                                        (CSRV_WRITE_HANDLER)NULL_PTR,
                                        (CSRV_TIMEOUT_HANDLER)cxmpp_csocket_cnode_timeout,
                                        (CSRV_CLOSE_HANDLER)cxmpp_csocket_cnode_shutdown);
    if(NULL_PTR == TASK_BRD_CSRV(task_brd))
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_start_cxmpp_srv: start cxmpp srv failed\n");
        return (EC_FALSE);
    }
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_start_cxmpp_srv: start server at port %ld\n", cxmpp_srv_port);    
    return (EC_TRUE);
}

EC_BOOL task_brd_default_start_cxmpp_srv(const UINT32 crfs_md_id, const UINT32 cxmpp_srv_ipaddr, const UINT32 cxmpp_srv_port)
{
    TASK_BRD *task_brd;

    task_brd = task_brd_default_get();
    return task_brd_start_cxmpp_srv(task_brd, crfs_md_id, cxmpp_srv_ipaddr, cxmpp_srv_port);
}
...
EC_BOOL task_brd_default_start_cextsrv(const UINT32 srv_ipaddr, const UINT32 srv_port, const UINT32 thread_num)
{
    ...
    cextsrv = cextsrv_start(srv_ipaddr, srv_port);
    if(NULL_PTR == cextsrv)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_default_start_cextsrv: start cextsrv on %s:%ld failed\n", 
    ...
    for(thread_pos = 0; thread_pos < thread_num; thread_pos ++)
    {
        ...
        if(NULL_PTR == croutine_node)
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_brd_default_start_cextsrv: cthread load cextsrv_thread failed\n");
            continue;
        }
        CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_TASK_0215);
    }
...
EC_BOOL task_brd_net_add_runner(const UINT32 tcid, const UINT32 mask_nbits, const UINT32 rank, TASK_RUNNER runner)
{
     ...
     )
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 1)(LOGSTDOUT, "warn:task_brd_net_add_runner: tcid %s rank %ld runner would be overrided!\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));
        }
        else
        {
            dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_net_add_runner: tcid %s rank %ld runner was set\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));        
        }
...
EC_BOOL task_brd_range_add_runner(const UINT32 tcid_fr, const UINT32 tcid_to, const UINT32 rank, TASK_RUNNER runner)
{
     ...
     )
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 1)(LOGSTDOUT, "warn:task_brd_range_add_runner: tcid %s rank %ld runner would be overrided!\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));
        }
        else
        {
            dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_range_add_runner: tcid %s rank %ld runner was set\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));        
        }
...
EC_BOOL task_brd_default_add_runner(const UINT32 tcid, const UINT32 rank, TASK_RUNNER runner)
{
     ...
     )
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 1)(LOGSTDOUT, "warn:task_brd_default_add_runner: tcid %s rank %ld runner would be overrided!\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));
        }
        else
        {
            dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "task_brd_default_add_runner: tcid %s rank %ld runner was set\n",
                               TASK_BRD_TCID_STR(task_brd), TASK_BRD_RANK(task_brd));        
        }
...
EC_BOOL do_slave(TASK_BRD *task_brd)
{
    ...
    for(;;)
    {
        ...
        {  
            ...
            {
                dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:do_slave: tasks_do_once found fatal error\n");
                tasks_srv_end(TASK_BRD_TASKS_CFG(task_brd));
        ...
        if(NULL_PTR != TASK_BRD_CSRV(task_brd))
        {
            ...
            {
                dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:do_slave: csrv_do_once found fatal error\n");
        ...
        {
            dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "[DEBUG] do_slave: tasks_monitor_empty_flag is false\n");
            if(EC_TRUE == tasks_monitor_is_empty(TASKS_CFG_MONITOR(tasks_cfg)))
            {
                dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "[DEBUG] do_slave: set tasks_monitor_empty_flag to true\n");
        ...
        if(0)/*debug*/
        {
            ...
            )
            {
                ...
                dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: recving %ld, is_recv %ld, to_send %ld, sending %ld, "
                                   "rcv_task_mgr %ld, aging_task_mgr %ld, req routine %ld, rsp routine %ld\n",
                                    recving_num, is_recv_num, to_send_num, sending_num, 
                                    recv_task_mgr_num, aging_task_mgr_num,
...
#endif/*(SWITCH_OFF == TASK_BRD_CEPOLL_SWITCH)*/
#if 0
        if(CPU_OVERLOAD_THREASHOLD < task_brd_cpu_load_get(task_brd))
        {
            /*when overload, go to sleep*/
            g_do_slave_usleep_counter ++;
            c_usleep(TASK_SLOW_DOWN_MSEC, LOC_TASK_0236);
        }
        //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: busy...\n");
#endif        
    }
...
EC_BOOL do_slave_enhanced(TASK_BRD *task_brd)
{
    TASKS_CFG  *tasks_cfg;
    EC_BOOL     tasks_monitor_empty_flag;
    EC_BOOL     task_brd_register_cluster_flag;

    COROUTINE_POOL *coroutine_pool;
    COROUTINE_NODE *coroutine_node_master;
    
    dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "do_slave_enhanced is running on tid %d\n", CTHREAD_GET_TID());
    ...
    task_brd_register_cluster_flag = EC_FALSE;
    ...
    for(;;)
    {
        ...
        dbg_log(SEC_0015_TASK, 9)(LOGSTDNULL, "[DEBUG] do_slave_enhanced: [0]\n");
        
        /*update task_brd time*/
        task_brd_update_time(task_brd);

        dbg_log(SEC_0015_TASK, 9)(LOGSTDNULL, "[DEBUG] do_slave_enhanced: [1]\n");
        ...
        if(CMPI_FWD_RANK == TASK_BRD_RANK(task_brd))
        {
            ...
            if(EC_FALSE == tasks_do_once(TASK_BRD_TASKS_CFG(task_brd)))
            {
                dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:do_slave_enhanced: tasks_do_once found fatal error\n");
                tasks_srv_end(TASK_BRD_TASKS_CFG(task_brd));
                task_brd_default_abort();/*abort taskcomm*/
            }
#endif            
            tasks_work_isend(TASKS_CFG_WORK(tasks_cfg));
            tasks_work_heartbeat(TASKS_CFG_WORK(tasks_cfg));
        }
        ...
        if(NULL_PTR != TASK_BRD_CSRV(task_brd))
        {
            if(EC_FALSE == csrv_do_once(TASK_BRD_CSRV(task_brd)))
            {
                dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:do_slave_enhanced: csrv_do_once found fatal error\n");
        ...
        dbg_log(SEC_0015_TASK, 9)(LOGSTDNULL, "[DEBUG] do_slave_enhanced: [5]\n");

        slow_down_flag = EC_TRUE;/*default is to slow down*/ 

        if(1)/*set slow down flag*/
        {
            UINT32 recving_num;
            UINT32 is_recv_num;
            UINT32 to_send_num;
            ...
            )
            {
                ...
                dbg_log(SEC_0015_TASK, 9)(LOGSTDNULL, "[DEBUG] do_slave_enhanced: recving %ld, is_recv %ld, to_send %ld, sending %ld, "
                                   "rcv_task_mgr %ld, aging_task_mgr %ld, req routine %ld, rsp routine %ld\n",
                                    recving_num, is_recv_num, to_send_num, sending_num, 
                                    recv_task_mgr_num, aging_task_mgr_num,
                                    req_croutine_num, rsp_croutine_num);
        ...
        else
        {
            ...
            {
                g_do_slave_usleep_counter ++;
                c_usleep(TASK_SLOW_DOWN_MSEC, LOC_TASK_0237);
            }
        }
#endif/*(SWITCH_ON == TASK_BRD_CEPOLL_SWITCH)*/

        /*------------------------------------------------------------------------*/
        coroutine_pool_run_once(coroutine_pool, coroutine_node_master);
    }
...
#if 0
EC_BOOL do_slave_0(TASK_BRD *task_brd)
{
    for(;;)
    {
        /* check if we caught some signals and process them */
...
#if (SWITCH_OFF == TASK_BRD_CEPOLL_SWITCH)
        //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: active-counter %u\n", TASK_BRD_DEFAULT_ACTIVE_COUNTER());

        if(TASK_BRD_DEFAULT_ACTIVE_COUNTER_IS_NOT_SET())
        {
            /*when inactive, go to sleep*/
            //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: Zzzzzzzzzz...\n");
            //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] slow down %u msec\n", TASK_SLOW_DOWN_MSEC);
            g_do_slave_usleep_counter ++;
            c_usleep(TASK_SLOW_DOWN_MSEC, LOC_TASK_0238);
            //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] wakeup\n");
        }
#endif/*(SWITCH_OFF == TASK_BRD_CEPOLL_SWITCH)*/
#if 0
        if(CPU_OVERLOAD_THREASHOLD < task_brd_cpu_load_get(task_brd))
        {
            /*when overload, go to sleep*/
            g_do_slave_usleep_counter ++;
            c_usleep(TASK_SLOW_DOWN_MSEC, LOC_TASK_0239);
        }
        //dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: busy...\n");
#endif
        ...
        TASK_BRD_DEFAULT_ACTIVE_COUNTER_CLR(LOC_TASK_0240);/*set to inactive*/
        ...
        if(0)/*debug*/
        {
            ...
            {
                dbg_log(SEC_0015_TASK, 9)(LOGSTDOUT, "[DEBUG] do_slave: recving %ld, is_recv %ld, to_send %ld, sending %ld, rcv_task_mgr %ld, aging_task_mgr %ld\n",
...
EC_BOOL do_slave_wait_default()
{
    ...
    while(ERR_CTHREAD_ID != TASK_BRD_DO_SLAVE_CTHREAD_ID(task_brd))/*reset automatically*/
    {
        ...
        {
            dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "do_slave_wait_default: NOT restart do_slave thread and quit now\n");
            break;
        }

        dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "do_slave_wait_default: try to restart do_slave thread ....\n");
        ...
        dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT,"do_slave_wait_default: do_slave thread %u\n", TASK_BRD_DO_SLAVE_CTHREAD_ID(task_brd));
...
UINT32 task_act(const MOD_MGR *src_mod_mgr, MOD_MGR **des_mod_mgr, const UINT32 time_to_live, const UINT32 mod_num, const UINT32 load_balancing_choice, const UINT32 task_prio, const UINT32 func_id, ...)
{
    ...
    for(mod_idx = 0; mod_idx < mod_num; mod_idx ++)
    {
        ...
        task_req = task_req_new(0, TASK_MGR_SEQNO(task_mgr), sub_seqno, TASK_ACT_TYPE, task_mgr, LOC_TASK_0242);
...
UINT32 task_dea(MOD_MGR *mod_mgr, const UINT32 time_to_live, const UINT32 task_prio, const UINT32 func_id, ...)
{
    ...
    for(pos = 0; pos < MOD_MGR_REMOTE_NUM(mod_mgr); pos ++)
    {
        ...
        task_req = task_req_new(0, TASK_MGR_SEQNO(task_mgr), sub_seqno, TASK_DEA_TYPE, task_mgr, LOC_TASK_0243);
...
EC_BOOL task_wait0(TASK_MGR *task_mgr)
{
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "fatal error:task_wait: something wrong, jumping disordered\n");
        exit(0);
    }

    /*otherwise, longjump back and continue execution next codes*/
    dbg_log(SEC_0015_TASK, 5)(LOGSTDOUT, "================================= task %ld  end %lx ================================================\n", TASK_MGR_SEQNO(task_mgr), task_mgr);
...
EC_BOOL task_wait(TASK_MGR *task_mgr, const UINT32 time_to_live, const UINT32 task_reschedule_flag, CHECKER ret_val_checker)
{
    ...
    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "================================= task %lx.%lx.%lx  start %lx [tid %d] ==============================================\n",
    ...
    {
        dbg_log(SEC_0015_TASK, 0)(LOGDEBUG, "error:task_wait: current tid %d, do_slave_pid %d\n", CTHREAD_GET_TID(), TASK_BRD_DO_SLAVE_PID(task_brd));    
    }
    ASSERT(CTHREAD_GET_TID() != TASK_BRD_DO_SLAVE_PID(task_brd));
#endif/*(SWITCH_ON == CROUTINE_SUPPORT_CTHREAD_SWITCH)*/

                        
    TASK_MGR_CCOND_RESERVE(task_mgr, 1, LOC_TASK_0244);

    task_brd_task_mgr_add(task_brd, task_mgr);
    TASK_MGR_CCOND_WAIT(task_mgr, LOC_TASK_0245);

    /*when reach here, task is done*/

    dbg_log(SEC_0015_TASK, 3)(LOGDEBUG, "================================= task %lx.%lx.%lx  end %lx ================================================\n",
    ...
    dbg_log(SEC_0015_TASK, 2)(LOGDEBUG, "task_wait report: start at %4d-%02d-%02d %02d:%02d:%02d.%03d, end at %4d-%02d-%02d %02d:%02d:%02d.%03d, "
...
TASK_MGR * task_new(const MOD_MGR *mod_mgr, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num)
{
    ...
    alloc_static_mem(MD_TASK, 0, MM_TASK_MGR, &task_mgr, LOC_TASK_0246);
...
UINT32 task_pos_inc(TASK_MGR *task_mgr, const UINT32 recv_mod_node_pos, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(recv_mod_node_pos >= MOD_MGR_REMOTE_NUM(mod_mgr))
    {
        ...
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_pos_inc: mod_mgr %lx remote mod nodes list: \n", mod_mgr);
    ...
    task_req = task_req_new(0, task_seqno, sub_seqno, TASK_NORMAL_TYPE, task_mgr, LOC_TASK_0256);
...
UINT32 task_pos_mono(const MOD_MGR *mod_mgr, const UINT32 time_to_live, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num, const UINT32  recv_mod_node_pos, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(recv_mod_node_pos >= MOD_MGR_REMOTE_NUM(mod_mgr))
    {
        ...
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_pos_mono: mod_mgr %lx remote mod nodes list: ", mod_mgr);
...
UINT32 task_pos_mono_no_wait(const MOD_MGR *mod_mgr, const UINT32 time_to_live, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num, const UINT32  recv_mod_node_pos, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(recv_mod_node_pos >= MOD_MGR_REMOTE_NUM(mod_mgr))
    {
        ...
        dbg_log(SEC_0015_TASK, 0)(LOGSTDOUT, "error:task_pos_mono_no_wait: mod_mgr %lx remote mod nodes list: ", mod_mgr);
        cvector_print(LOGSTDOUT, MOD_MGR_REMOTE_LIST(mod_mgr), (CVECTOR_DATA_PRINT)mod_node_print);
        return ((UINT32)(-1));
    }
...
UINT32 task_tcid_inc(TASK_MGR *task_mgr, const UINT32 recv_tcid, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(NULL_PTR == recv_mod_node)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDERR, "error:task_tcid_inc: no mod_node with tcid %s in mod_mgr %lx\n", c_word_to_ipv4(recv_tcid), mod_mgr);
        return ((UINT32)(-1));
    }
...
UINT32 task_tcid_mono(const MOD_MGR *mod_mgr, const UINT32 time_to_live, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num, const UINT32 recv_tcid, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(NULL_PTR == recv_mod_node)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDERR, "error:task_tcid_mono: no mod_node with tcid %s in mod_mgr %lx\n", c_word_to_ipv4(recv_tcid), mod_mgr);
        return ((UINT32)(-1));
    }
...
UINT32 task_tcid_mono_no_wait(const MOD_MGR *mod_mgr, const UINT32 time_to_live, const UINT32 task_prio, const UINT32 task_need_rsp_flag, const UINT32 task_need_rsp_num, const UINT32 recv_tcid, const void * func_retval_addr, const UINT32 func_id, ...)
{
    ...
    if(NULL_PTR == recv_mod_node)
    {
        dbg_log(SEC_0015_TASK, 0)(LOGSTDERR, "error:task_tcid_mono_no_wait: no mod_node with tcid %s in mod_mgr %lx\n", c_word_to_ipv4(recv_tcid), mod_mgr);
        return ((UINT32)(-1));
    }
