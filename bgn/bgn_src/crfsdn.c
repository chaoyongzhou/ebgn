...
void crfsdn_node_fname_print(LOG *log, const CRFSDN *crfsdn, const UINT32 node_id)
{
    ...
    sys_log(log, "${ROOT}/dsk%ld/%08ld/%08ld\n",
                disk_no,
                CRFSDN_PATH_LAYOUT_DIR0_NO(block_no),
                CRFSDN_PATH_LAYOUT_DIR1_NO(block_no)
                );
    return;
}
...
static EC_BOOL __crfsdn_node_fname_gen(const CRFSDN *crfsdn, const UINT32 node_id, char *path, const UINT32 max_len)
{
    ...
    snprintf(path, max_len, "%s/dsk%ld/%08ld/%08ld",
...
static uint16_t __crfsdn_count_disk_num_from_disk_space(const uint16_t max_gb_num_of_disk_space)
{
    ...
    dbg_log(SEC_0024_CRFSDN, 9)(LOGSTDOUT, "[DEBUG] __crfsdn_count_disk_num_from_disk_space: ### set 1 disk = %u MB for debug purpose \n", CPGD_DEBUG_MB_PER_DISK);
    ...
    dbg_log(SEC_0024_CRFSDN, 9)(LOGSTDOUT, "[DEBUG] __crfsdn_count_disk_num_from_disk_space: ### set 1 disk = %u GB for debug purpose \n", CPGD_DEBUG_GB_PER_DISK);
...
CRFSDN *crfsdn_new()
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CRFSDN, &crfsdn, LOC_CRFSDN_0031);
    if(NULL_PTR != crfsdn)
    {
        crfsdn_init(crfsdn);
        return (crfsdn);
...
EC_BOOL crfsdn_free(CRFSDN *crfsdn)
{
    ...
    {
        crfsdn_clean(crfsdn);
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CRFSDN, crfsdn, LOC_CRFSDN_0038);
    }
    return (EC_TRUE);
}
...
EC_BOOL crfsdn_read_o(CRFSDN *crfsdn, const uint16_t disk_no, const uint16_t block_no, const UINT32 offset, const UINT32 data_max_len, UINT8 *data_buff, UINT32 *data_len)
{
    ...
    node_id = CRFSDN_NODE_ID_MAKE(disk_no, block_no);
    offset_t = offset;
    //dbg_log(SEC_0024_CRFSDN, 9)(LOGSTDOUT, "[DEBUG] crfsdn_read_o: disk %u, block %u  ==> node %u\n", disk_no, block_no, node_id);
