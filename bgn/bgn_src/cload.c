...
CLOAD_STAT *cload_stat_new()
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLOAD_STAT, &cload_stat, LOC_CLOAD_0001);
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_stat_new: new cload stat failed\n");
        return (NULL_PTR);
    }
...
EC_BOOL cload_stat_free(CLOAD_STAT *cload_stat)
{
    if(NULL_PTR != cload_stat)
    {
        ...
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLOAD_STAT, cload_stat, LOC_CLOAD_0002);
    }
    return (EC_TRUE);
}
...
EC_BOOL cload_stat_init_0(const UINT32 md_id, CLOAD_STAT *cload_stat)
{
    return cload_stat_init(cload_stat);
}

EC_BOOL cload_stat_clean_0(const UINT32 md_id, CLOAD_STAT *cload_stat)
{
    return cload_stat_clean(cload_stat);
}

EC_BOOL cload_stat_free_0(const UINT32 md_id, CLOAD_STAT *cload_stat)
{
    return cload_stat_free(cload_stat);
}
...
CLOAD_NODE *cload_node_new(const UINT32 tcid, const UINT32 comm, const UINT32 size)
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLOAD_NODE, &cload_node, LOC_CLOAD_0003);
    if(NULL_PTR == cload_node)
    {
        sys_log(LOGSTDOUT, "error:cload_node_new: new cload node failed\n");
        return (NULL_PTR);
    }
...
EC_BOOL cload_node_free(CLOAD_NODE *cload_node)
{
    if(NULL_PTR != cload_node)
    {
        ...
        free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLOAD_NODE, cload_node, LOC_CLOAD_0006);
    }
    return (EC_TRUE);
}
...
EC_BOOL cload_node_update(const CLOAD_NODE *cload_node_src, CLOAD_NODE *cload_node_des)
{
    ...
    {
        sys_log(LOGSTDOUT, "error:cload_node_update: tcid not matched: src tcid %s, des tcid %s\n",
    ...
    if(src_size <= des_size)
    {
        for(pos = 0; pos < src_size; pos ++)
        {
            if(local_tcid != CLOAD_NODE_TCID(cload_node_des) || local_rank != pos)/*not update local rank load*/
            {
                ...
                //sys_log(LOGSTDOUT, "[DEBUG] cload_node_update[1]: load_src %ld, load_des %ld => %ld\n", load_src, load_des, CLOAD_MAKE(load_des, load_src));
            }
        }
    }
    else
...
EC_BOOL cload_node_init_0(const UINT32 md_id, CLOAD_NODE *cload_node)
{
    return cload_node_init(cload_node, CMPI_ERROR_TCID, CMPI_ERROR_COMM, 0);
}

EC_BOOL cload_node_clean_0(const UINT32 md_id, CLOAD_NODE *cload_node)
{
    return cload_node_clean(cload_node);
}

EC_BOOL cload_node_free_0(const UINT32 md_id, CLOAD_NODE *cload_node)
{
    return cload_node_free(cload_node);
}
...
CLOAD_STAT * cload_node_get(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get: rank overflow where tcid %s, rank %ld but size %ld\n",
...
EC_BOOL cload_node_set(CLOAD_NODE *cload_node, const UINT32 rank, const CLOAD_STAT *cload_stat_src)
{
    ...
    //sys_log(LOGSTDOUT, "[DEBUG] cload_node_set_que: try to set tcid %s rank %ld load %ld\n", CLOAD_NODE_TCID_STR(cload_node), rank, load);
    ...
    sys_log(LOGSTDOUT, "error:cload_node_set_que: unknown issue where tcid %s, rank %ld but rank load vec size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
    return (EC_FALSE);
}
...
EC_BOOL cload_node_set_que(CLOAD_NODE *cload_node, const UINT32 rank, const UINT32 que_load)
{
    ...
    sys_log(LOGSTDOUT, "error:cload_node_set_que: unknown issue where tcid %s, rank %ld, que load %ld but rank load vec size %ld\n",
...
UINT32 cload_node_get_que(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_que: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
EC_BOOL cload_node_inc_que(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    sys_log(LOGSTDOUT, "error:cload_node_inc_que: unknown issue where tcid %s, rank %ld, but rank load vec size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
    return (EC_FALSE);
}
...
EC_BOOL cload_node_dec_que(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    sys_log(LOGSTDOUT, "error:cload_node_dec_que: unknown issue where tcid %s, rank %ld, but rank load vec size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
    return (EC_FALSE);
}
...
UINT32 cload_node_get_obj(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_obj: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
UINT32 cload_node_get_cpu(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_cpu: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
UINT32 cload_node_get_mem(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_mem: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
UINT32 cload_node_get_dsk(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_dsk: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
UINT32 cload_node_get_net(CLOAD_NODE *cload_node, const UINT32 rank)
{
    ...
    if(NULL_PTR == cload_stat)
    {
        sys_log(LOGSTDOUT, "error:cload_node_get_net: rank overflow where tcid %s, rank %ld but size %ld\n",
                    CLOAD_NODE_TCID_STR(cload_node), rank, cvector_size(CLOAD_NODE_RANK_LOAD_STAT_VEC(cload_node)));
        return ((UINT32)0);
    }
...
CLIST *cload_mgr_new()
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLIST, &cload_mgr, LOC_CLOAD_0007);
...
EC_BOOL cload_mgr_free(CLIST *cload_mgr)
{
    ...
    free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLIST, cload_mgr, LOC_CLOAD_0009);
    return (EC_TRUE);
}

EC_BOOL cload_mgr_init_0(const UINT32 md_id, CLOAD_MGR *cload_mgr)
{
    return cload_mgr_init(cload_mgr);
}

EC_BOOL cload_mgr_clean_0(const UINT32 md_id, CLOAD_MGR *cload_mgr)
{
    return cload_mgr_clean(cload_mgr);
}

EC_BOOL cload_mgr_free_0(const UINT32 md_id, CLOAD_MGR *cload_mgr)
{
    return cload_mgr_free(cload_mgr);
}
...
EC_BOOL cload_mgr_add(CLIST *cload_mgr, const CLOAD_NODE *cload_node)
{
    ...
    {
        sys_log(LOGSTDOUT, "error:cload_mgr_add: cload node of tcid %s already exist\n", CLOAD_NODE_TCID_STR(cload_node));
        return (EC_FALSE);
    }
...
EC_BOOL cload_mgr_rmv(CLIST *cload_mgr, const CLOAD_NODE *cload_node)
{
    ...
    {
        sys_log(LOGSTDOUT, "error:cload_mgr_rmv: cload node of tcid %s not exist\n", CLOAD_NODE_TCID_STR(cload_node));
        return (EC_FALSE);
    }
