...
CHASH_NODE *chash_node_new(const UINT32 klen, const UINT32 vlen, const UINT8 *key, const UINT8 *value)
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CHASH_NODE, &chash_node, LOC_CHASHNODE_0001);
...
EC_BOOL chash_node_free(CHASH_NODE *chash_node)
{
    ...
    free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CHASH_NODE, chash_node, LOC_CHASHNODE_0002);
    return (EC_TRUE);
}
...
CLIST *chash_list_new()
{
    ...
    alloc_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLIST, &chash_list, LOC_CHASHNODE_0003);
...
EC_BOOL chash_list_free(CLIST *chash_list)
{
    ...
    free_static_mem(MD_TASK, CMPI_ANY_MODI, MM_CLIST, chash_list, LOC_CHASHNODE_0005);
    return (EC_TRUE);
}
