...
void tasks_cfg_init(TASKS_CFG *tasks_cfg)
{
    ...
    tasks_work_init(TASKS_CFG_WORK(tasks_cfg));
...
void tasks_cfg_free(TASKS_CFG *tasks_cfg)
{
    ...
    free_static_mem(MD_TASK, 0, MM_TASKS_CFG, tasks_cfg, LOC_TASKCFG_0006);
    return;
}
...
EC_BOOL tasks_cfg_clone(const TASKS_CFG *src_tasks_cfg, TASKS_CFG *des_tasks_cfg)
{
    ...
    /*NOTE: do not clone srvsockfd, tasks_work and tasks_monitor which are run-time dynamic information*/
    return (EC_TRUE);
}
...
void tasks_cfg_print(LOG *log, const TASKS_CFG *tasks_cfg)
{
    ...
    sys_log(log, "tasks_cfg: work clients");
    cvector_print(log, TASKS_CFG_WORK(tasks_cfg), (CVECTOR_DATA_PRINT)tasks_node_print);

    sys_log(log, "tasks_cfg: monitor clients");
    cvector_print(log, TASKS_CFG_MONITOR(tasks_cfg), (CVECTOR_DATA_PRINT)csocket_cnode_print);
...
void taskr_cfg_free(TASKR_CFG *taskr_cfg)
{
    ...
    free_static_mem(MD_TASK, 0, MM_TASKR_CFG, taskr_cfg, LOC_TASKCFG_0012);
    return;
}
...
void task_cfg_free(TASK_CFG *task_cfg)
{
    ...
    free_static_mem(MD_TASK, 0, MM_TASK_CFG, task_cfg, LOC_TASKCFG_0016);
    return;
}
