...
static int __crfschttp_on_headers_complete(http_parser_t* http_parser) 
{
    CRFSCHTTP_NODE *crfschttp_node;
...
void crfschttp_node_defer_close(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    cepoll_set_event(cepoll, CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_WR_EVENT,
                      (CEPOLL_EVENT_HANDLER)crfschttp_csocket_cnode_epoll_close,(void *)csocket_cnode);    
...
void crfschttp_csocket_cnode_timeout(CSOCKET_CNODE *csocket_cnode)
{
    ...
    crfschttp_node_defer_close(csocket_cnode);
    return;
}
...
EC_BOOL crfschttp_defer_launch()
{
...
EC_BOOL crfschttp_handle_setsmf_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    if(EC_FALSE == chunk_mgr_export_to_cbytes(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node), content_cbytes))
    {
        ...
        cstring_clean(&path_cstr);
        cbytes_free(content_cbytes, LOC_CRFSCHTTP_0005);
        return (EC_TRUE);
    }    
    ...
    if(EC_TRUE == __crfschttp_uri_is_setsmf_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_write(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0006);
    ...
    cstring_clean(&path_cstr);
    cbytes_free(content_cbytes, LOC_CRFSCHTTP_0007);    
...
EC_BOOL crfschttp_handle_update_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    if(EC_FALSE == chunk_mgr_export_to_cbytes(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node), content_cbytes))
    {
        ...
        cstring_clean(&path_cstr);
        cbytes_free(content_cbytes, LOC_CRFSCHTTP_0008);
        return (EC_TRUE);
    }    
    ...
    else if(EC_TRUE == __crfschttp_uri_is_update_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_update(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0009);
            return (EC_TRUE);
        }
    }    
    else
    ...
    cstring_clean(&path_cstr);
    cbytes_free(content_cbytes, LOC_CRFSCHTTP_0010);    
...
EC_BOOL crfschttp_handle_put_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    if(EC_FALSE == chunk_mgr_export_to_cbytes(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node), content_cbytes))
    {
        ...
        cstring_clean(&path_cstr);
        cbytes_free(content_cbytes, LOC_CRFSCHTTP_0011);
        return (EC_TRUE);
    }    
    ...
    if(EC_TRUE == __crfschttp_uri_is_setsmf_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_write(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0012);
            return (EC_TRUE);
        }
    } 
    ...
    else if(EC_TRUE == __crfschttp_uri_is_update_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_update(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0013);
            return (EC_TRUE);
        }
    }    
    else
    ...
    cstring_clean(&path_cstr);
    cbytes_free(content_cbytes, LOC_CRFSCHTTP_0014);    
...
EC_BOOL crfschttp_handle_post_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    if(EC_FALSE == chunk_mgr_export_to_cbytes(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node), content_cbytes))
    {
        ...
        cstring_clean(&path_cstr);
        cbytes_free(content_cbytes, LOC_CRFSCHTTP_0015);
        return (EC_TRUE);
    }    
    ...
    if(EC_TRUE == __crfschttp_uri_is_setsmf_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_write(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0016);
            return (EC_TRUE);
        }
    } 
    ...
    else if(EC_TRUE == __crfschttp_uri_is_update_op(uri_cbuffer))
    {
        ...
        if(EC_FALSE == crfsc_update(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, expired_nsec))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_free(content_cbytes, LOC_CRFSCHTTP_0017);
            return (EC_TRUE);
        }
    }
    ...
    cstring_clean(&path_cstr);
    cbytes_free(content_cbytes, LOC_CRFSCHTTP_0018);    
...
static EC_BOOL __crfschttp_handle_getrgf_request_fetch_path(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    store_path = cstring_new(NULL_PTR, LOC_CRFSCHTTP_0019);
...
EC_BOOL crfschttp_handle_getrgf_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    content_cbytes = CRFSCHTTP_NODE_CONTENT_CBYTES(crfschttp_node);
    cbytes_clean(content_cbytes, LOC_CRFSCHTTP_0020);
...
EC_BOOL crfschttp_handle_getsmf_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    content_cbytes = CRFSCHTTP_NODE_CONTENT_CBYTES(crfschttp_node);
    cbytes_clean(content_cbytes, LOC_CRFSCHTTP_0021);
    ...
    if(NULL_PTR != store_offset_str)
    {
        ...
        if(EC_FALSE == crfsc_read_e(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, &offset, max_len, content_cbytes, &expires_timestamp, expired_body_needed))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_clean(content_cbytes, LOC_CRFSCHTTP_0022);                                
            return (EC_TRUE);
        }
        CRFSCHTTP_NODE_RSP_STATUS(crfschttp_node) = CRFSCHTTP_OK;        
    ...
    else/*read whole file content*/
    {
        ...
        if(EC_FALSE == crfsc_read(CSOCKET_CNODE_MODI(csocket_cnode), &path_cstr, content_cbytes, &expires_timestamp, expired_body_needed))
        {
            ...
            cstring_clean(&path_cstr);
            cbytes_clean(content_cbytes, LOC_CRFSCHTTP_0023);                                
            return (EC_TRUE);
        }
        CRFSCHTTP_NODE_RSP_STATUS(crfschttp_node) = CRFSCHTTP_OK;
    }
...
{       
    ...
    if(NULL_PTR == CRFSCHTTP_NODE_SEND_DATA_BUFF(crfschttp_node))
    {
        ...
        data_buff = safe_malloc(max_len, LOC_CRFSCHTTP_0024);
...
static EC_BOOL __crfschttp_handle_getbgf_request_fetch_path(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    store_path = cstring_new(NULL_PTR, LOC_CRFSCHTTP_0025);
    if(NULL_PTR == store_path)
    {
...
EC_BOOL crfschttp_handle_getbgf_request(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    ASSERT(0 == chunk_mgr_count_chunks(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node)));
    content_cbytes = CRFSCHTTP_NODE_CONTENT_CBYTES(crfschttp_node);
    cbytes_clean(content_cbytes, LOC_CRFSCHTTP_0026);
...
{    
    ...
    chunk_mgr_clean(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node));
    cbytes_clean(CRFSCHTTP_NODE_CONTENT_CBYTES(crfschttp_node), LOC_CRFSCHTTP_0027);
...
EC_BOOL crfschttp_parse_content_length(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    CRFSCHTTP_NODE_CONTENT_LENGTH(crfschttp_node) = c_chars_to_uint64((char *)cstring_get_str(content_length_cstr), 
    ...
    if(HTTP_GET == http_parser->method)
    {
        ...
        if(NULL_PTR == croutine_node)
        {
            ...
            cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT,
                  (CEPOLL_EVENT_HANDLER)crfschttp_commit_request, (void *)crfschttp_node);    

            return (EC_FALSE);
        }
        CSOCKET_CNODE_RETRIES(csocket_cnode) = 0;
        CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CRFSCHTTP_0028);    
...
CRFSCHTTP_NODE *crfschttp_node_new(const uint32_t size)
{
    ...
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CRFSCHTTP_NODE, &crfschttp_node, LOC_CRFSCHTTP_0031);
    if(NULL_PTR == crfschttp_node)
    {
    ...
    if(EC_FALSE == crfschttp_node_init(crfschttp_node, size))
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CRFSCHTTP_NODE, crfschttp_node, LOC_CRFSCHTTP_0032);
        return (NULL_PTR);
    }
...
EC_BOOL crfschttp_node_clean(CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    cbytes_clean(CRFSCHTTP_NODE_CONTENT_CBYTES(crfschttp_node), LOC_CRFSCHTTP_0033);
    ...
    if(NULL_PTR != CRFSCHTTP_NODE_SEND_DATA_BUFF(crfschttp_node))
    {
        safe_free(CRFSCHTTP_NODE_SEND_DATA_BUFF(crfschttp_node), LOC_CRFSCHTTP_0034);
        CRFSCHTTP_NODE_SEND_DATA_BUFF(crfschttp_node) = NULL_PTR;
    }
    CRFSCHTTP_NODE_SEND_DATA_TOTAL_LEN(crfschttp_node) = 0;
    CRFSCHTTP_NODE_SEND_DATA_SENT_LEN(crfschttp_node)  = 0; 
...
EC_BOOL crfschttp_node_free(CRFSCHTTP_NODE *crfschttp_node)
{
    if(NULL_PTR != crfschttp_node)
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CRFSCHTTP_NODE, crfschttp_node, LOC_CRFSCHTTP_0035);
    }
...
void crfschttp_node_print(LOG *log, const CRFSCHTTP_NODE *crfschttp_node)
{
    ...
    sys_log(LOGSTDOUT, "crfschttp_node_print:node %p, header content length: %"PRIx64"\n", crfschttp_node, 
                       CRFSCHTTP_NODE_CONTENT_LENGTH(crfschttp_node));

    sys_log(LOGSTDOUT, "crfschttp_node_print:node %p, req body chunks: total length %"PRIx64"\n", crfschttp_node, 
                       chunk_mgr_total_length(CRFSCHTTP_NODE_REQ_BODY_CHUNKS(crfschttp_node)));
                       
    sys_log(LOGSTDOUT, "crfschttp_node_print:node %p, rsp body chunks: total length %"PRIx64"\n", crfschttp_node, 
...
EC_BOOL crfschttp_recv_on_csocket_cnode(CSOCKET_CNODE *csocket_cnode)
{
    ...
    if(0 == recv_len)
    {
        ...
        cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT,
                          (CEPOLL_EVENT_HANDLER)crfschttp_recv_on_csocket_cnode_thread, (void *)csocket_cnode);
        return (EC_TRUE);
    }
...
*/
    ...
    if(NULL_PTR != CRFSCHTTP_NODE_SEND_DATA_MORE(crfschttp_node))
    {
        ...
        cepoll_set_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_WR_EVENT,
                         (CEPOLL_EVENT_HANDLER)CRFSCHTTP_NODE_SEND_DATA_MORE(crfschttp_node), (void *)crfschttp_node);
        return (EC_TRUE);
    }
...
EC_BOOL crfschttp_recv_on_csocket_cnode_thread(CSOCKET_CNODE *csocket_cnode)
{
    ...
    cepoll_del_event(task_brd_default_get_cepoll(), CSOCKET_CNODE_SOCKFD(csocket_cnode), CEPOLL_RD_EVENT);
    CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CRFSCHTTP_0036);    
...
EC_BOOL crfschttp_send_on_csocket_cnode_thread(CSOCKET_CNODE *csocket_cnode)
{
    ...
    CROUTINE_NODE_COND_RELEASE(croutine_node, LOC_CRFSCHTTP_0037);    
