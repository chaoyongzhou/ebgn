...
EC_BOOL kbuff_init_0(const UINT32 md_id, KBUFF *kbuff)
{
    KBUFF_CACHE(kbuff)     = NULL_PTR;
    KBUFF_MAX_LEN(kbuff)   = 0;
    KBUFF_CUR_LEN(kbuff)   = 0;
...
EC_BOOL kbuff_clean_0(const UINT32 md_id, KBUFF *kbuff)
{
    if(NULL_PTR != KBUFF_CACHE(kbuff))
    {
...
void kbuff_free_0(const UINT32 md_id, KBUFF *kbuff)
{
    kbuff_free(kbuff);
    return;
}
...
EC_BOOL kbuff_fread(KBUFF *kbuff, const UINT32 size, FILE *fp)
{
    ...
    if(0 == KBUFF_MAX_LEN(kbuff))
    {
        kbuff_init(kbuff, FILE_SEG_MAX_SIZE);
    }
