...
CBTREE_KEY *cbtree_key_new(const CBTREE *cbtree)
{
    ...
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBTREE_KEY, &cbtree_key, LOC_CBTREE_0004);
...
CBTREE_NODE *cbtree_node_new(const CBTREE *cbtree)
{
    ...
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBTREE_NODE, &cbtree_node, LOC_CBTREE_0011);
...
EC_BOOL cbtree_free(CBTREE *cbtree)
{
    ...
    {
        cbtree_clean(cbtree);
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CBTREE, cbtree, LOC_CBTREE_0014);
    }
    return (EC_TRUE);
}
...
static CBTREE_KEY * __cbtree_search0(CBTREE *cbtree, CBTREE_NODE *root_node, const CBTREE_KEY *key)
{
    uint8_t i;
    int result;
    ...
    return __cbtree_search0(cbtree, CBTREE_NODE_CHILD(root_node, i), key);
}
...
static EC_BOOL __cbtree_split_push_all_0(CBTREE *cbtree, CLIST *cached_key_list)
{    
    while(EC_FALSE == clist_is_empty(cached_key_list))
    {
        ...
        /*keep the original version order!*/
        for(ver = CBTREE_MAX_VER(cbtree); ver -- > 0;)
        {
            uint8_t *key;

            key = CBTREE_KEY_KV(del_key, ver);
            if(NULL_PTR != key)
            {
                cbtree_insert(cbtree, key);
            }
        }
...
CBTREE * cbtree_load_hsdfs(const UINT32 cdfs_md_id, const CSTRING *fname_cstr)
{
    ...
    encoded_buf = (uint8_t *)safe_malloc(encoded_len, LOC_CBTREE_0042);
    ...
    safe_free(encoded_buf, LOC_CBTREE_0048);
