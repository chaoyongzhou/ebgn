...
EC_BOOL crfsnp_mgr_free(CRFSNP_MGR *crfsnp_mgr)
{
    if(NULL_PTR != crfsnp_mgr)
    {
        ...
        free_static_mem(MD_CRFS, CMPI_ANY_MODI, MM_CRFSNP_MGR, crfsnp_mgr, LOC_CRFSNPMGR_0008);
    }
    return (EC_TRUE);
}
...
static EC_BOOL __crfsnp_mgr_node_size(CRFSNP_MGR *crfsnp_mgr, CRFSNP *crfsnp, uint32_t node_pos, uint64_t *file_size)
{
    ...
    else if(CRFSNP_ITEM_FILE_IS_BIG == CRFSNP_ITEM_DIR_FLAG(item))
    {
        ...
        dbg_log(SEC_0009_CRFSNPMGR, 9)(LOGSTDOUT, "[DEBUG] __crfsnp_mgr_node_size: bnode fsize = %llu\n", CRFSNP_BNODE_FILESZ(crfsnp_bnode));
    ...
    {
        dbg_log(SEC_0009_CRFSNPMGR, 0)(LOGSTDOUT, "error:__crfsnp_mgr_node_size: invalid dflg %lx\n", CRFSNP_ITEM_DIR_FLAG(item));
        return (EC_FALSE);
    }
...
static EC_BOOL __crfsnp_mgr_dir_size(CRFSNP_MGR *crfsnp_mgr, uint32_t crfsnp_id, const CRFSNP_DNODE *crfsnp_dnode, uint64_t *file_size)
{
    ...
    if(NULL_PTR == crfsnp)
    {
        dbg_log(SEC_0009_CRFSNPMGR, 0)(LOGSTDOUT, "error:__crfsnp_mgr_dir_size: open np %u failed\n", crfsnp_id);
        return (EC_FALSE);
    }
...
EC_BOOL crfsnp_mgr_file_size(CRFSNP_MGR *crfsnp_mgr, const CSTRING *path_cstr, uint64_t *file_size)
{
    ...
    if(CRFSNPRB_ERR_POS == node_pos)
    {
        dbg_log(SEC_0009_CRFSNPMGR, 0)(LOGSTDOUT, "error:crfsnp_mgr_file_size: get size of file %s\n", (char *)cstring_get_str(path_cstr));
        return (EC_FALSE);
    }    
...
EC_BOOL crfsnp_mgr_recycle_np(CRFSNP_MGR *crfsnp_mgr, const uint32_t crfsnp_id, CRFSNP_RECYCLE_DN *crfsnp_recycle_dn)
{
    CRFSNP  *crfsnp;
