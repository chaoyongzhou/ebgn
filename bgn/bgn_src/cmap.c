...
{
    CMAP_NODE *cmap_node;
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CMAP_NODE, &cmap_node, location);
...
EC_BOOL    cmap_node_free(CMAP_NODE *cmap_node, CMAP_KEY_FREE key_free, CMAP_VAL_FREE val_free, const UINT32 location)
{
    if(NULL_PTR != cmap_node)
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CMAP_NODE, cmap_node, location);
    }
    return (EC_TRUE);
}
...
{
    CMAP *cmap;
    alloc_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CMAP, &cmap, location);
    if(NULL_PTR != cmap)
    {
...
EC_BOOL cmap_free(CMAP *cmap, const UINT32 location)
{
    if(NULL_PTR != cmap)
    {
        ...
        free_static_mem(MD_TBD, CMPI_ANY_MODI, MM_CMAP, cmap, location);
    }
    return (EC_TRUE);
}
