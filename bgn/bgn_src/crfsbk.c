...
CRFSOP *crfsop_new()
{
    ...
    alloc_static_mem(MD_CRFS, CMPI_ANY_MODI, MM_CRFSOP, &crfsop, LOC_CRFSBK_0001);
    if(NULL_PTR == crfsop)
    {
...
EC_BOOL crfsop_free(CRFSOP *crfsop)
{
    ...
    {
        crfsop_clean(crfsop);
        free_static_mem(MD_CRFS, CMPI_ANY_MODI, MM_CRFSOP, crfsop, LOC_CRFSBK_0002);
    }
    return (EC_TRUE);
}

EC_BOOL crfsop_init_0(const UINT32 md_id, CRFSOP *crfsop)
{
    return crfsop_init(crfsop);
}

EC_BOOL crfsop_clean_0(const UINT32 md_id, CRFSOP *crfsop)
{
    return crfsop_clean(crfsop);
}

EC_BOOL crfsop_free_0(const UINT32 md_id, CRFSOP *crfsop)
{
    return crfsop_free(crfsop);
}
...
/*for debug only*/
static REAL __crfsbk_room_ratio(CRFSBK *crfsbk)
{
    CRFSDN   *crfsdn;
    CPGV     *cpgv;
    double    ratio;
    ...
    ratio = (CPGV_PAGE_4K_USED_NUM(cpgv) + 0.0) / (CPGV_PAGE_4K_MAX_NUM(cpgv)  + 0.0);
    return (ratio);
}
...
EC_BOOL crfsbk_room_is_ok_no_lock(CRFSBK *crfsbk, const REAL level)
{
    ...
    used_size = (((uint64_t)CPGV_PAGE_4K_USED_NUM(cpgv)) << CPGB_PAGE_4K_BIT_SIZE);
    max_size  = (((uint64_t)CPGV_PAGE_4K_MAX_NUM(cpgv)) << CPGB_PAGE_4K_BIT_SIZE);
...
EC_BOOL crfsbk_write_no_lock(CRFSBK *crfsbk, const CSTRING *file_path, const CBYTES *cbytes, const uint32_t expire_nsec, const uint8_t *md5sum)
{
    ...
    if(EC_FALSE == crfsbk_reserve_dn_no_lock(crfsbk, size, &disk_no, &block_no, &page_4k_no))
    {   
    ...
    /*add to diskcache*/
    offset  = (((UINT32)(page_4k_no)) << (CPGB_PAGE_4K_BIT_SIZE));
    if(EC_FALSE == crfsdn_write_o(CRFSBK_DN(crfsbk), cbytes_len(cbytes), CBYTES_BUF(cbytes), disk_no, block_no, &offset))
    {
        dbg_log(SEC_0141_CRFSBK, 0)(LOGSTDOUT, "error:crfsbk_write_no_lock: write %u bytes to disk %u block %u page %u failed\n", 
                            cbytes_len(cbytes), disk_no, block_no, page_4k_no);
    ...
    dbg_log(SEC_0141_CRFSBK, 9)(LOGSTDOUT, "[DEBUG] crfsbk_write_no_lock: file %s write to disk %u block %u page %u done\n", 
                       (char *)cstring_get_str(file_path), disk_no, block_no, page_4k_no);    
...
EC_BOOL crfsbk_read_dn_no_lock(CRFSBK *crfsbk, const CRFSNP_FNODE *crfsnp_fnode, CBYTES *cbytes)
{
    ...
    {
        dbg_log(SEC_0024_CRFSDN, 0)(LOGSTDOUT, "error:crfsbk_read_dn_no_lock: read %u bytes from disk %u block %u page %u failed\n", 
...
EC_BOOL crfsbk_recycle_no_lock(CRFSBK *crfsbk)
{
    CRFSNP     *crfsnp;
...
static CRFSNP_ITEM * __crfsbk_search_item(CRFSBK *crfsbk, const uint32_t path_len, const uint8_t *path, const uint32_t dflag)
{
    ...
    if(CRFSNPRB_ERR_POS == node_pos)
    {
        dbg_log(SEC_0031_CRFS, 9)(LOGSTDOUT, "[DEBUG] __crfsbk_search_item: not found path %.*s\n", 
                            path_len, path);
        return (NULL_PTR);
    }
...
EC_BOOL crfsbk_qfile(CRFSBK *crfsbk, const CSTRING *file_path, CRFSNP_ITEM  *crfsnp_item)
{
    ...
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_qfile: query file %s failed\n", 
                            (char *)cstring_get_str(file_path));
        return (EC_FALSE);
    }
...
EC_BOOL crfsbk_qdir(CRFSBK *crfsbk, const CSTRING *dir_path, CRFSNP_ITEM  *crfsnp_item)
{
    ...
    if(NULL_PTR == crfsnp_item_src)
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_qdir: query dir %s failed\n", 
...
EC_BOOL crfsbk_recycle(CRFSBK *crfsbk)
{
    CRFSBK_WRLOCK(crfsbk, LOC_CRFSBK_0025);
    if(EC_FALSE == crfsbk_recycle_no_lock(crfsbk))
    {
...
EC_BOOL crfsbk_replay_file(CRFSBK *crfsbk, const CSTRING *path)
{
    ...
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: new master crfsnp_item failed\n");
        return (EC_FALSE);
    }
    ...
    if(EC_TRUE == crfs_qfile(CRFSBK_CRFS_MD_ID(crfsbk), path, crfsnp_item_master))
    {
        ...
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: new backup crfsnp_item failed\n");
        ...
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: qfile %s in backup RFS failed\n",
        ...
        {
            dbg_log(SEC_0031_CRFS, 9)(LOGSTDOUT, "[DEBUG] crfsbk_replay_file: file %s in master RFS is new, give up replaying\n",
                                (char *)cstring_get_str(path));
            crfsnp_item_free(crfsnp_item_master);
            crfsnp_item_free(crfsnp_item_backup);
            return (EC_TRUE);
        ...
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: delete file %s from master RFS failed\n",
                                (char *)cstring_get_str(path));
            crfsnp_item_free(crfsnp_item_master);
            return (EC_FALSE);
        }
        dbg_log(SEC_0031_CRFS, 9)(LOGSTDOUT, "[DEBUG] crfsbk_replay_file: rmv file %s in master RFS\n",
                            (char *)cstring_get_str(path));         
    }
    ...
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: read file %s from backup RFS failed\n",
                            (char *)cstring_get_str(path));
        cbytes_clean(&cbytes, LOC_CRFSBK_0031);
        return (EC_FALSE);
    }
    ...
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file: replay file %s to master RFS failed\n",
                            (char *)cstring_get_str(path));
        cbytes_clean(&cbytes, LOC_CRFSBK_0032);
        return (EC_FALSE);
    }

    cbytes_clean(&cbytes, LOC_CRFSBK_0033);

    //crfsbk_remove_file(crfsbk, path);

    dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "[DEBUG] crfsbk_replay_file: replay file %s done\n",
                        (char *)cstring_get_str(path));    
    return (EC_TRUE);
}
...
{
    dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_file_b: not support replay big file yet\n");
    return (EC_FALSE);
}
...
EC_BOOL crfsbk_replay_rm_dir_op(CRFSBK *crfsbk, CRFSOP *crfsop)
{
    ...
    if(NULL_PTR == crfsnp_item_master)
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_rm_dir_op: new master crfsnp_item failed\n");
        return (EC_FALSE);
    }
    ...
    if(EC_TRUE == crfs_qdir(CRFSBK_CRFS_MD_ID(crfsbk), path, crfsnp_item_master))
    {
        ...
        if(NULL_PTR == crfsnp_item_backup)
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_rm_dir_op: new backup crfsnp_item failed\n");
            crfsnp_item_free(crfsnp_item_master);
            return (EC_FALSE);
        }
        ...
        if(EC_FALSE == crfsbk_qfile(crfsbk, path, crfsnp_item_backup))
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_rm_dir_op: dir %s in backup RFS failed\n",
                                (char *)cstring_get_str(path));
            crfsnp_item_free(crfsnp_item_master);
            crfsnp_item_free(crfsnp_item_backup);
            return (EC_FALSE);
        ...
        if(CRFSNP_ITEM_CREATE_TIME(crfsnp_item_master) > CRFSNP_ITEM_CREATE_TIME(crfsnp_item_backup))
        {
            dbg_log(SEC_0031_CRFS, 9)(LOGSTDOUT, "[DEBUG] crfsbk_replay_rm_dir_op: dir %s in master RFS is new, give up replaying\n",
                                (char *)cstring_get_str(path));
            crfsnp_item_free(crfsnp_item_master);
            crfsnp_item_free(crfsnp_item_backup);
            return (EC_TRUE);
        ...
        dbg_log(SEC_0031_CRFS, 9)(LOGSTDOUT, "[DEBUG] crfsbk_replay_rm_dir_op: rmv dir %s in master RFS\n",
                            (char *)cstring_get_str(path));        
    }
...
EC_BOOL crfsbk_replay_rm_big_op(CRFSBK *crfsbk, CRFSOP *crfsop)
{
    ...
    dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_rm_big_op: not support replay RM_BIG op yet\n");
    return (EC_FALSE);
}
...
EC_BOOL crfsbk_replay_one(CRFSBK *crfsbk, CRFSOP *crfsop)
{
    ...
    dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay_one: invalid op type 0x%x\n", CRFSOP_OP_TYPE(crfsop));
    return (EC_FALSE);
}
...
EC_BOOL crfsbk_replay(CRFSBK *crfsbk)
{
    ...
    if(NULL_PTR == CRFSBK_NP(crfsbk))
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay: npp was not open\n");
        return (EC_FALSE);
    }

    if(NULL_PTR == CRFSBK_DN(crfsbk))
    {
        dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "error:crfsbk_replay: dn was not open\n");
        return (EC_FALSE);
    }
    ...
    for(;;)
    {
        ...
        {
            dbg_log(SEC_0031_CRFS, 0)(LOGSTDOUT, "crfsbk_replay: replay crfsop %p failed where op %s, path [%s] %s, hash %x\n", crfsop, 
                         __crfsop_op_type(CRFSOP_OP_TYPE(crfsop)),
                         __crfsop_path_type(CRFSOP_PATH_TYPE(crfsop)),
                         CRFSOP_PATH_STR(crfsop),
                         CRFSOP_PATH_HASH(crfsop)); 
