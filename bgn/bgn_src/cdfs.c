...
UINT32 cdfs_start(const UINT32 cdfsnp_min_num)
{
    ...
    if(ERR_MODULE_ID == cdfs_md_id)
    {
        return (ERR_MODULE_ID);
    }

    /* initilize new one CDFS module */
...
static UINT32 cdfs_tcid_filter_out(const CVECTOR *tcid_vec, const UINT32 tcid)
{
...
static EC_BOOL cdfs_collect_fnode_succ_tcid(const CDFSNP_FNODE *cdfsnp_fnode, CVECTOR *tcid_vec)
{
    UINT32 cdfsnp_inode_pos;
...
static EC_BOOL cdfs_get_first_succ_inode_pos_0(const CDFSNP_FNODE *cdfsnp_fnode, UINT32 *cdfsnp_inode_first_pos)
{
    TASK_BRD *task_brd;
    UINT32 cdfsnp_inode_pos;
        ...
        )
    {
        if(CDFSNP_ERR_PATH != CDFSNP_FNODE_INODE_PATH(cdfsnp_fnode, cdfsnp_inode_pos) &&
           CDFSNP_ERR_FOFF != CDFSNP_FNODE_INODE_FOFF(cdfsnp_fnode, cdfsnp_inode_pos)&&
           EC_TRUE == task_brd_check_tcid_connected(task_brd, CDFSNP_FNODE_INODE_TCID(cdfsnp_fnode, cdfsnp_inode_pos))
           )
        {
            (*cdfsnp_inode_first_pos) = cdfsnp_inode_pos;
            return (EC_TRUE);
        }
    }
...
static EC_BOOL cdfs_get_next_succ_inode_pos_0(const CDFSNP_FNODE *cdfsnp_fnode, const UINT32 cdfsnp_inode_beg_pos, UINT32 *cdfsnp_inode_next_pos)
{
    TASK_BRD *task_brd;
    UINT32 cdfsnp_inode_pos;
...
static EC_BOOL cdfs_get_first_succ_inode_pos(const UINT32 cdfs_md_id, const CDFSNP_FNODE *cdfsnp_fnode, UINT32 *cdfsnp_inode_first_pos)
{
    UINT32 cdfsnp_inode_pos;
    ...
    {
        sys_log(LOGSTDOUT,
                "error:cdfs_get_first_succ_inode_pos: cdfs module #0x%lx not started.\n",
                cdfs_md_id);
        cdfs_print_module_status(cdfs_md_id, LOGSTDOUT);
        dbg_exit(MD_CDFS, cdfs_md_id);
    }
...
static MOD_MGR *cdfs_new_dn_mod_mgr_to_write(const UINT32 cdfs_md_id)
{
    ...
    for(mod_node_pos = 0; mod_node_pos < cvector_size(MOD_MGR_REMOTE_LIST(mod_mgr_src)); mod_node_pos ++)
    {
        ...
        if(CDFS_MOD_NODE_WRITE_DISABLE != MOD_NODE_STAT(mod_node_src))
        {
            ...
            alloc_static_mem(MD_CDFS, cdfs_md_id, MM_MOD_NODE, &mod_node_des, LOC_CDFS_0004);
...
CDFSNP_FNODE *cdfs_fnode_new(const UINT32 cdfs_md_id)
{
    return cdfsnp_fnode_new();
}

EC_BOOL cdfs_fnode_init(const UINT32 cdfs_md_id, CDFSNP_FNODE *cdfsnp_fnode)
{
    return cdfsnp_fnode_init(cdfsnp_fnode);
}

EC_BOOL cdfs_fnode_clean(const UINT32 cdfs_md_id, CDFSNP_FNODE *cdfsnp_fnode)
{
    return cdfsnp_fnode_clean(cdfsnp_fnode);
}

EC_BOOL cdfs_fnode_free(const UINT32 cdfs_md_id, CDFSNP_FNODE *cdfsnp_fnode)
{
    return cdfsnp_fnode_free(cdfsnp_fnode);
}
...
EC_BOOL cdfs_exists_npp(const UINT32 cdfs_md_id, const CSTRING *path)
{
    ...
    task_inc(task_mgr, &ret, FI_cdfs_exists, ERR_MODULE_ID, path);
...
EC_BOOL cdfs_truncate_dn_ppl(const UINT32 cdfs_md_id, const UINT32 fsize, const UINT32 cdfsnp_inode_pos, CDFSNP_FNODE *cdfsnp_fnode, CDFSDN_STAT *cdfsdn_stat)
{
    ...
    if(EC_TRUE == cdfsdn_is_full(CDFS_MD_DN(cdfs_md)))
    {
        if(NULL_PTR != CDFS_MD_DN_MOD_MGR(cdfs_md) && 0 < MOD_MGR_REMOTE_NUM(CDFS_MD_DN_MOD_MGR(cdfs_md)))
        {
            task_bcast(CDFS_MD_DN_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_HIGH, TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP,
                       FI_cdfs_disable_write_access_dn, ERR_MODULE_ID, CMPI_LOCAL_TCID);
        }
    ...
    if(cdfsnp_inode_pos + 1 < CDFSNP_FNODE_REPNUM(cdfsnp_fnode) && cdfsnp_inode_pos + 1 < CDFSNP_FILE_REPLICA_MAX_NUM)
    {
        ...
        task_inc(task_mgr, &ret, FI_cdfs_truncate_dn_ppl, ERR_MODULE_ID, fsize, cdfsnp_inode_pos + 1, cdfsnp_fnode, &remote_cdfsdn_stat);
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_truncate_dn_p(const UINT32 cdfs_md_id, const UINT32 fsize, const UINT32 replica_num, CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
         ...
         **/
        task_mgr = task_new(mod_mgr, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
        task_inc(task_mgr, &ret, FI_cdfs_truncate_dn_ppl, ERR_MODULE_ID, fsize, (UINT32)0, cdfsnp_fnode, &remote_cdfsdn_stat);
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_update_dn_p(const UINT32 cdfs_md_id, const CBYTES *cbytes, const CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
        ...
        task_p2p(cdfs_md_id, TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                    &recv_mod_node,
                    &ret, FI_cdfs_update_dn_ppl, ERR_MODULE_ID, cbytes, cdfsdn_inode_pos, cdfsnp_fnode);
    }
...
EC_BOOL cdfs_update_dn_ppl(const UINT32 cdfs_md_id, const CBYTES *cbytes, const UINT32 cdfsnp_inode_pos, const CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(cdfsnp_inode_pos + 1 < CDFSNP_FNODE_REPNUM(cdfsnp_fnode) && cdfsnp_inode_pos + 1 < CDFSNP_FILE_REPLICA_MAX_NUM)
    {
        ...
        task_p2p(cdfs_md_id, TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                    &recv_mod_node,
                    &ret, FI_cdfs_update_dn_ppl, ERR_MODULE_ID, cbytes, cdfsnp_inode_pos + 1, cdfsnp_fnode);
...
EC_BOOL cdfs_write_dn_ppl(const UINT32 cdfs_md_id, const CBYTES *cbytes, const UINT32 cdfsnp_inode_pos, CDFSNP_FNODE *cdfsnp_fnode, CDFSDN_STAT *cdfsdn_stat)
{
    ...
    if(cdfsnp_inode_pos + 1 < CDFSNP_FNODE_REPNUM(cdfsnp_fnode) && cdfsnp_inode_pos + 1 < CDFSNP_FILE_REPLICA_MAX_NUM)
    {
        ...
        task_mgr = task_new(mod_mgr, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
        task_inc(task_mgr, &ret, FI_cdfs_write_dn_ppl, ERR_MODULE_ID, cbytes, cdfsnp_inode_pos + 1, cdfsnp_fnode, &remote_cdfsdn_stat);
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_read_dn_ppl(const UINT32 cdfs_md_id, const UINT32 cdfsnp_inode_pos, const CDFSNP_FNODE *cdfsnp_fnode, CBYTES *cbytes)
{
    ...
    if(EC_FALSE == cdfsdn_read(CDFS_MD_DN(cdfs_md), path_layout, offset, file_size, CBYTES_BUF(cbytes), &(CBYTES_LEN(cbytes))))
    {
        ...
        if(EC_TRUE == cdfs_get_next_succ_inode_pos(cdfs_md_id, cdfsnp_fnode, cdfsnp_inode_pos, &cdfsnp_inode_next_pos))
        {
            ...
            cbytes_clean(cbytes, LOC_CDFS_0032);

            task_super_mono(CDFS_MD_DN_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                            &recv_mod_node,
                            &ret, FI_cdfs_read_dn_ppl, ERR_MODULE_ID, cdfsnp_inode_next_pos, cdfsnp_fnode, cbytes);
        }
        return (ret);
    }
#if 0
...
EC_BOOL cdfs_write_dn_p(const UINT32 cdfs_md_id, const CBYTES *cbytes, const UINT32 replica_num, CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
         ...
         **/
        task_mgr = task_new(mod_mgr, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
        task_inc(task_mgr, &ret, FI_cdfs_write_dn_ppl, ERR_MODULE_ID, cbytes, (UINT32)0, cdfsnp_fnode, &remote_cdfsdn_stat);
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_read_dn_p_with_tcid_filter(const UINT32 cdfs_md_id, const CVECTOR *cdfsdn_tcid_vec, const CDFSNP_FNODE *cdfsnp_fnode, CBYTES *cbytes)
{
    ...
    ret = EC_FALSE;
    task_mgr = task_new(mod_mgr, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
    task_inc(task_mgr, &ret, FI_cdfs_read_dn, ERR_MODULE_ID, cdfsnp_fnode, cbytes);
    task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_read_dn_p(const UINT32 cdfs_md_id, const CDFSNP_FNODE *cdfsnp_fnode, CBYTES *cbytes)
{
    ...
    cdfsdn_tcid_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0033);
    ...
    cvector_free(cdfsdn_tcid_vec, LOC_CDFS_0034);
...
EC_BOOL cdfs_read_dn(const UINT32 cdfs_md_id, const CDFSNP_FNODE *cdfsnp_fnode, CBYTES *cbytes)
{
    ...
    if(CBYTES_LEN(cbytes) < file_size)
    {
        if(NULL_PTR != CBYTES_BUF(cbytes))
        {
            SAFE_FREE(CBYTES_BUF(cbytes), LOC_CDFS_0035);
        }
        CBYTES_BUF(cbytes) = (UINT8 *)SAFE_MALLOC(file_size, LOC_CDFS_0036);
        CBYTES_LEN(cbytes) = 0;
    }
...
EC_BOOL cdfs_write_npp_p(const UINT32 cdfs_md_id, const CSTRING *file_path, const UINT32 replica_num, const CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
        ...
        ret_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0037);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            alloc_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, &ret, LOC_CDFS_0038);
            cvector_push_no_lock(ret_vec, (void *)ret);

            (*ret) = EC_FALSE;

            task_pos_inc(task_mgr, remote_mod_node_idx, ret, FI_cdfs_write_npp, ERR_MODULE_ID, file_path, cdfsnp_fnode);
        }
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NOT_NEED_RESCHEDULE_FLAG, NULL_PTR);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            free_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, ret, LOC_CDFS_0039);
        }

        cvector_free_no_lock(ret_vec, LOC_CDFS_0040);
    }
    ...
    if(succ_counter < CDFS_MD_NP_MIN_NUM(cdfs_md))/*when np replica not meet requirement, record it in log*/
    {
        ...
        if(0 < MOD_MGR_REMOTE_NUM(CDFS_MD_NPP_MOD_MGR(cdfs_md)))
        {
            ...
            task_mono(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_HIGH,
                        ...
                        NULL_PTR, FI_cdfs_lost_fnode_log, ERR_MODULE_ID, file_path, cdfsnp_fnode);
#endif
#if 0
            TASK_MGR *task_mgr;
            task_new(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_PRIO_HIGH, TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP, &task_mgr);
            task_inc(task_mgr, NULL_PTR, FI_cdfs_lost_fnode_log, ERR_MODULE_ID, file_path, cdfsnp_fnode);
            task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
#endif
        }
        else
    ...
    if(replica_num > CDFSNP_FNODE_REPNUM(cdfsnp_fnode))/*when dn replica not meet requirement, record it in log*/
    {
        ...
        if(0 < MOD_MGR_REMOTE_NUM(CDFS_MD_NPP_MOD_MGR(cdfs_md)))
        {
            ...
            task_mono(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_HIGH,
                        TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP, TASK_NEED_RESCHEDULE_FLAG,
                        NULL_PTR, FI_cdfs_lost_replica_log, ERR_MODULE_ID, file_path, replica_num, cdfsnp_fnode);
#endif
#if 0
            TASK_MGR *task_mgr;
            task_new(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_PRIO_HIGH, TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP, &task_mgr);
            task_inc(task_mgr, NULL_PTR, FI_cdfs_lost_replica_log, ERR_MODULE_ID, file_path, replica_num, cdfsnp_fnode);
            task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
#endif
        }
        else
...
EC_BOOL cdfs_read_npp_p(const UINT32 cdfs_md_id, const CSTRING *file_path, CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
        ...
        task_mgr = task_new(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
        task_inc(task_mgr, &ret, FI_cdfs_read_npp, ERR_MODULE_ID, file_path, cdfsnp_fnode);
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
    }
...
EC_BOOL cdfs_update_npp_p(const UINT32 cdfs_md_id, const CSTRING *file_path, const CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    if(1)
    {
        ...
        ret_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0041);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            alloc_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, &ret, LOC_CDFS_0042);
            cvector_push(ret_vec, (void *)ret);

            (*ret) = EC_FALSE;

            task_pos_inc(task_mgr, remote_mod_node_idx, ret, FI_cdfs_update_npp, ERR_MODULE_ID, file_path, cdfsnp_fnode);
        }
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NOT_NEED_RESCHEDULE_FLAG, NULL_PTR);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            free_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, ret, LOC_CDFS_0043);
        }

        cvector_free(ret_vec, LOC_CDFS_0044);
    }
...
EC_BOOL cdfs_update_npp_p(const UINT32 cdfs_md_id, const CSTRING *file_path, const CDFSNP_FNODE *cdfsnp_fnode)
{
    ...
    ret_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0045);
    ...
    for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
    {
        ...
        alloc_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, &ret, LOC_CDFS_0046);
        cvector_push_no_lock(ret_vec, (void *)ret);
    ...
    for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
    {
        ...
        cvector_set_no_lock(ret_vec, remote_mod_node_idx, NULL_PTR);
        free_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, ret, LOC_CDFS_0047);
    }
    cvector_free_no_lock(ret_vec, LOC_CDFS_0048);
...
EC_BOOL cdfs_delete_npp_p(const UINT32 cdfs_md_id, const CSTRING *path, const UINT32 dflag, CVECTOR *cdfsnp_fnode_vec)
{
    ...
    if(1)/*delete from all npp*/
    {
        ...
        collected_vec = cvector_new(0, MM_CVECTOR, LOC_CDFS_0049);
        ret_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0050);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            cdfsnp_fnode_vec_ret = cvector_new(0, MM_CDFSNP_FNODE, LOC_CDFS_0051);
            cvector_push_no_lock(collected_vec, (void *)cdfsnp_fnode_vec_ret);

            alloc_static_mem(MD_CDFS, cdfs_md_id, MM_UINT32, &ret, LOC_CDFS_0052);
            cvector_push_no_lock(ret_vec, (void *)ret);

            (*ret) = EC_FALSE;

            task_pos_inc(task_mgr, remote_mod_node_idx, ret, FI_cdfs_delete_npp, ERR_MODULE_ID, path, dflag, cdfsnp_fnode_vec_ret);
        }
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NOT_NEED_RESCHEDULE_FLAG, NULL_PTR);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            cvector_set_no_lock(ret_vec, remote_mod_node_idx, NULL_PTR);
            free_static_mem(MD_CDFS, cdfs_md_id, MM_UINT32, ret, LOC_CDFS_0053);

            cvector_set_no_lock(collected_vec, remote_mod_node_idx, NULL_PTR);
            cvector_clean_no_lock(cdfsnp_fnode_vec_ret, (CVECTOR_DATA_CLEANER)cdfsnp_fnode_free, LOC_CDFS_0054);
            cvector_free_no_lock(cdfsnp_fnode_vec_ret, LOC_CDFS_0055);
        }

        cvector_free_no_lock(ret_vec, LOC_CDFS_0056);
    ...
    for(cdfsnp_fnode_pos = 0; cdfsnp_fnode_pos < cvector_size(cdfsnp_fnode_vec); cdfsnp_fnode_pos ++)
    {
        ...
        for(replica_pos = 0; replica_pos < CDFSNP_FNODE_REPNUM(cdfsnp_fnode); replica_pos ++)
        {
            ...
            task_super_inc(task_mgr, &send_mod_node, &recv_mod_node,
                            &(ret[replica_pos]), FI_cdfs_delete_dn, ERR_MODULE_ID, path_layout, file_offset);
        }
...
EC_BOOL cdfs_delete(const UINT32 cdfs_md_id, const CSTRING *path, const UINT32 dflag)
{
    ...
    cdfsnp_fnode_vec = cvector_new(0, MM_CDFSNP_FNODE, LOC_CDFS_0058);
    ...
    cvector_clean(cdfsnp_fnode_vec, (CVECTOR_DATA_CLEANER)cdfsnp_fnode_free, LOC_CDFS_0063);
    cvector_free(cdfsnp_fnode_vec, LOC_CDFS_0064);
    return (EC_TRUE);
}
...
EC_BOOL cdfs_qlist_seg_npp(const UINT32 cdfs_md_id, const CSTRING *file_seg, CVECTOR  *seg_cstr_vec)
{
    ...
    task_mgr = task_new(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
    task_inc(task_mgr, &ret, FI_cdfs_qlist_seg, ERR_MODULE_ID, file_seg, seg_cstr_vec);
    task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
...
EC_BOOL cdfs_flush_npp(const UINT32 cdfs_md_id, const UINT32 cdfsnpp_tcid)
{
    ...
    task_super_mono(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                    &recv_mod_node,
                    &ret, FI_cdfs_flush_npp, ERR_MODULE_ID, cdfsnpp_tcid);
    return (ret);
}
...
EC_BOOL cdfs_flush_dn(const UINT32 cdfs_md_id, const UINT32 cdfsdn_tcid)
{
    ...
    task_super_mono(CDFS_MD_DN_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                    &recv_mod_node,
                    &ret, FI_cdfs_flush_dn, ERR_MODULE_ID, cdfsdn_tcid);
    return (ret);
}
...
EC_BOOL cdfs_mkdir_npp(const UINT32 cdfs_md_id, const CSTRING *path_cstr)
{
    ...
    if(1)
    {
        ...
        ret_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0065);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            alloc_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, &ret, LOC_CDFS_0066);
            cvector_push(ret_vec, (void *)ret);

            (*ret) = EC_FALSE;

            task_pos_inc(task_mgr, remote_mod_node_idx, ret, FI_cdfs_mkdir, ERR_MODULE_ID, path_cstr);
        }
        task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NOT_NEED_RESCHEDULE_FLAG, NULL_PTR);
        ...
        for(remote_mod_node_idx = 0; remote_mod_node_idx < remote_mod_node_num; remote_mod_node_idx ++)
        {
            ...
            cvector_set(ret_vec, remote_mod_node_idx, NULL_PTR);
            free_static_mem(MD_CDFS, CMPI_ANY_MODI, MM_UINT32, ret, LOC_CDFS_0067);
        }

        cvector_free(ret_vec, LOC_CDFS_0068);
    }
    ...
    if(succ_counter < CDFS_MD_NP_MIN_NUM(cdfs_md))/*when np replica not meet requirement, record it in log*/
    {
        ...
        if(0 < MOD_MGR_REMOTE_NUM(CDFS_MD_NPP_MOD_MGR(cdfs_md)))
        {
            task_mono(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_HIGH,
                        TASK_NOT_NEED_RSP_FLAG, TASK_NEED_NONE_RSP, TASK_NEED_RESCHEDULE_FLAG,
                        NULL_PTR, FI_cdfs_lost_fnode_log, ERR_MODULE_ID, path_cstr, cdfsnp_fnode);
        }
        else
        {
...
*/
    ...
    task_tcid_mono(CDFS_MD_DN_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                    des_datanode_tcid,
                    &ret, FI_cdfs_transfer_in, ERR_MODULE_ID, cdfsdn_record, cdfsdn_block, des_block_path_layout);
...
EC_BOOL cdfs_check_file_content(const UINT32 cdfs_md_id, const UINT32 path_layout, const UINT32 offset, const UINT32 file_size, const CSTRING *file_content_cstr)
{
    ...
    for(pos = 0; pos < len; pos ++)
    {
        if(buff[ pos ] != str[ pos ])
        {
            ...
            cbytes_free(cbytes, LOC_CDFS_0073);
            return (EC_FALSE);
        }
    }

    cbytes_free(cbytes, LOC_CDFS_0074);
    return (EC_TRUE);
}
    ...
    if(1)
    {
            ...
            )
        {
            ...
            task_tcid_inc(task_mgr, CDFSNP_INODE_TCID(cdfsnp_inode),
                          &(ret[cdfsnp_inode_pos]), FI_cdfs_check_file_content, ERR_MODULE_ID,
    ...
    while(fgets(str_line, CDFS_LOST_FNODE_LINE_MAX_SIZE, lost_fnode_fp))
    {
        ...
        else
        {
            ...
            task_tcid_mono(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_DEFAULT_LIVE, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP,
                           des_tcid,
                           &ret, FI_cdfs_write_npp, ERR_MODULE_ID, path, &cdfsnp_fnode);
        }
...
EC_BOOL cdfs_import_lost_replica_from_file(const UINT32 cdfs_md_id, const CSTRING *file_name, const UINT32 des_tcid)
{
    ...
    while(fgets(str_line, CDFS_LOST_REPLICA_LINE_MAX_SIZE, lost_replica_fp))
    {
        ...
        if(1)
        {
            ...
            cdfsdn_excl_tcid_vec = cvector_new(0, MM_UINT32, LOC_CDFS_0078);
            ...
            task_mgr = task_new(mod_mgr, TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
            task_inc(task_mgr, &ret, FI_cdfs_write_dn_ppl, ERR_MODULE_ID, cbytes, cdfsnp_inode_pos, &cdfsnp_fnode, &remote_cdfsdn_stat);
            task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);

            mod_mgr_free(mod_mgr);
            cbytes_free(cbytes, LOC_CDFS_0081);
...
/*---------------------------------------- interface to FUSE used by Client or Agent ----------------------------------------*/
/**
*
*   get attr of a file or path
*
**/
EC_BOOL cdfs_getattr(const UINT32 cdfs_md_id, const CSTRING *path, CFUSE_STAT *stat)
{
    CDFS_MD *cdfs_md;
    ...
    {
        sys_log(LOGSTDOUT,
                "error:cdfs_getattr: cdfs module #0x%lx not started.\n",
                cdfs_md_id);
        dbg_exit(MD_CDFS, cdfs_md_id);
    }
#endif/*CDFS_DEBUG_SWITCH*/
    ...
    return cdfsnp_mgr_getattr(CDFS_MD_NPP(cdfs_md), path, stat);
}

EC_BOOL cdfs_getattr_npp(const UINT32 cdfs_md_id, const CSTRING *path, CFUSE_STAT *stat)
{
    CDFS_MD *cdfs_md;
    ...
    {
        sys_log(LOGSTDOUT,
                "error:cdfs_getattr_npp: cdfs module #0x%lx not started.\n",
                cdfs_md_id);
        dbg_exit(MD_CDFS, cdfs_md_id);
    }
#endif/*CDFS_DEBUG_SWITCH*/
    ...
    task_mgr = task_new(CDFS_MD_NPP_MOD_MGR(cdfs_md), TASK_PRIO_NORMAL, TASK_NEED_RSP_FLAG, TASK_NEED_ALL_RSP);
    task_inc(task_mgr, &ret, FI_cdfs_getattr, ERR_MODULE_ID, path, stat);
    task_wait(task_mgr, TASK_DEFAULT_LIVE, TASK_NEED_RESCHEDULE_FLAG, NULL_PTR);
