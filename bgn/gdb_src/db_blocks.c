...
gdbNewBlock(GDatabase *db, uint8_t blockType, void *extra)
{
    ...
    MEM_CHECK(block = (GdbBlock *)SAFE_MALLOC(sizeof(GdbBlock), LOC_DB_0022));
...
gdbDestroyBlockNoLock(GdbBlock *block)
{
    ...
    if (block->detail != NULL)
    {
        ...
        {
            SAFE_FREE(block->detail, LOC_DB_0023);
        }
    }
    ...
    if (block->chain != NULL)
    {
        SAFE_FREE(block->chain, LOC_DB_0024);
    }
    SAFE_FREE(block, LOC_DB_0025);
}
...
gdbDestroyBlock(GdbBlock *block)
{
    ...
    if (block->detail != NULL)
    {
        ...
        {
            SAFE_FREE(block->detail, LOC_DB_0026);
        }
    }
    ...
    if (block->chain != NULL)
    {
        SAFE_FREE(block->chain, LOC_DB_0027);
    }
    SAFE_FREE(block, LOC_DB_0028);
}
...
gdbReadBlockHeaderNoLock(GDatabase *db, offset_t offset, uint8_t blockType)
{
    ...
    if (rawFileRead(db->idxRawFile, offset, header, GDB_BLOCK_HEADER_SIZE, 1, LOC_DB_0029) != 1)
    {
        return NULL;
    }

    /* Allocate memory for the block. */
    MEM_CHECK(block = (GdbBlock *)SAFE_MALLOC(sizeof(GdbBlock), LOC_DB_0030));
    memset(block, 0, sizeof(GdbBlock));
...
{
    GdbBlock *block;
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0032);
    block = gdbReadBlockHeaderNoLock(db, offset, blockType);
    gdbUnlockFreeBlockList(db, LOC_DB_0033);
    return block;
}
...
gdbWriteBlockHeader(GdbBlock *block)
{
    ...
    rawFileWrite(db->idxRawFile, block->offset, header, GDB_BLOCK_HEADER_SIZE, 1, LOC_DB_0034);
             ...
             void *extra)
{
    ...
    MEM_CHECK(block->chain = (offset_t *)SAFE_MALLOC(block->chainCount * sizeof(offset_t), LOC_DB_0038));
    ...
    /* Create the buffer. */
    MEM_CHECK(buffer = (uint8_t *)SAFE_MALLOC(block->dataSize, LOC_DB_0039));
    ...
    if (rawFileRead(db->idxRawFile, __offset, buffer, dataSize, 1, LOC_DB_0040) != 1)
    {
    ...
    if (block->next != 0)
    {
        ...
        while (nextOffset != 0)
        {
            ...
            rawFileSeek(db->idxRawFile, __offset, SEEK_SET);
            rawFileRead(db->idxRawFile, __offset, &nextOffset, sizeof(offset_t), 1, LOC_DB_0041);
            ...
            rawFileSeek(db->idxRawFile, __offset, SEEK_SET);
            rawFileRead(db->idxRawFile, __offset, buffer + pos, 1, dataSize, LOC_DB_0042);
            __offset += dataSize;
    ...
    gdbUnlockFreeBlockList(db, LOC_DB_0044);
    return block;
}
...
gdbWriteBlock(GdbBlock *block)
{
    ...
    else if (block->chainCount < oldChainCount)
    {
        ...
        MEM_CHECK(block->chain = (offset_t *)SAFE_MALLOC(block->chainCount * sizeof(offset_t), LOC_DB_0045));
    ...
    else if (block->chainCount > oldChainCount)
    {
        ...
        SAFE_FREE(newChain, LOC_DB_0047);
    }
    ...
    dataSize = (block->dataSize < block->multiple - GDB_BLOCK_HEADER_SIZE ?
                block->dataSize : block->multiple - GDB_BLOCK_HEADER_SIZE);
    rawFileWrite(db->idxRawFile, __offset, buffer, 1, dataSize, LOC_DB_0048);
    __offset += dataSize;
    ...
    else
    {
        ...
        MEM_CHECK(blockBuffer = (uint8_t *)SAFE_MALLOC(block->multiple, LOC_DB_0049));
        ...
        for (i = 1; i < block->chainCount; i++)
        {
            ...
            rawFileWrite(db->idxRawFile, __offset, blockBuffer, 1, block->multiple, LOC_DB_0050);
            __offset += block->multiple;

            pos += block->multiple - sizeof(offset_t);
        }

        SAFE_FREE(blockBuffer, LOC_DB_0051);
    }
    ...
    if (oldChainCount != 0)
    {
        ...
        {
            SAFE_FREE(oldChain, LOC_DB_0052);
        }
    }

    if (buffer != block->detail)
    {
        SAFE_FREE(buffer, LOC_DB_0053);
    }
}
...
gdbBlockTypeAt(GDatabase *db, offset_t offset)
{
    ...
    rawFileSeek(db->idxRawFile, offset, SEEK_SET);
    if (rawFileRead(db->idxRawFile, offset, &type, 1, 1, LOC_DB_0054) != 1)
    {
        return GDB_BLOCK_ANY; /* Um. Kind of an error? */
    }
    return type;
...
gdbReserveBlockChain(GDatabase *db, uint16_t count, uint8_t blockType)
{
    ...
    /* Create the chain. */
    MEM_CHECK(chain = (offset_t *)SAFE_MALLOC(count * sizeof(offset_t), LOC_DB_0055));

    /* Lock the free block list. */
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0056);
    ...
    if (result == 0)
    {
        gdbUnlockFreeBlockList(db, LOC_DB_0057);
    ...
    MEM_CHECK(newFreeBlocks = (GdbFreeBlock *)SAFE_MALLOC(blockCount * sizeof(GdbFreeBlock), LOC_DB_0058));
                  ...
                  uint8_t blockType)
{
    ...
    /* Lock the free block list. */
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0061);
    ...
    if (blockCount == 0)
    {
        ...
        /* Block list is empty. */
        MEM_CHECK(freeBlocks = (GdbFreeBlock *)SAFE_MALLOC(count * sizeof(GdbFreeBlock), LOC_DB_0062));
    ...
    MEM_CHECK(freeBlocks = (GdbFreeBlock *)SAFE_MALLOC((blockCount + count) * sizeof(GdbFreeBlock), LOC_DB_0065));
    ...
    gdbUnlockFreeBlockList(db, LOC_DB_0066);
    gdbFreeBlockList(freeBlocks);
