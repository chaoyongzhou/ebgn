...
btreeReadHeader(GdbBlock *block, const uint8_t *buffer, void *extra)
{
    ...
    MEM_CHECK(tree = (BTree *)SAFE_MALLOC(sizeof(BTree), LOC_BTREE_0021));
    ...
    BTREE_CRWLOCK_INIT(tree, LOC_BTREE_0022);
    return tree;
}
...
btreeWriteHeader(GdbBlock *block, uint8_t **buffer, uint32_t *size)
{
    ...
    *size = BTREE_HEADER_DATA_SIZE;
    ...
    MEM_CHECK(*buffer = (uint8_t *)SAFE_MALLOC(BTREE_HEADER_DATA_SIZE, LOC_BTREE_0023));
...
btreeCreateHeader(GdbBlock *block, void *extra)
{
    ...
    MEM_CHECK(tree = (BTree *)SAFE_MALLOC(sizeof(BTree), LOC_BTREE_0024));
    memset(tree, 0, sizeof(BTree));
    ...
    BTREE_CRWLOCK_INIT(tree, LOC_BTREE_0025);
...
btreeDestroyHeader(void *tree)
{
    if (NULL != tree)
    {
        BTREE_CRWLOCK_CLEAN((BTree *)tree, LOC_BTREE_0026);
        SAFE_FREE(tree, LOC_BTREE_0027);
    }
    return;
}
...
btreeSetRootNode(BTree *tree, offset_t offset)
{
    ...
    rawFileWrite(fp, __offset, &offset, sizeof(offset_t), 1, LOC_BTREE_0028);
}
...
btreeSetLeftLeaf(BTree *tree, offset_t offset)
{
    ...
    rawFileWrite(fp, __offset, &offset, sizeof(offset_t), 1, LOC_BTREE_0029);
}
...
btreeSetTreeSize(BTree *tree, uint32_t size)
{
    ...
    rawFileWrite(fp, __offset, &size, sizeof(uint32_t), 1, LOC_BTREE_0030);
}
...
btreeGetRootNode(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET;
    if (rawFileRead(fp, __offset, &root_offset, sizeof(offset_t), 1, LOC_BTREE_0031) != 1)
    {
...
btreeGetLeftLeaf(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET;
    if (rawFileRead(fp, __offset, &leaf_offset, sizeof(offset_t), 1, LOC_BTREE_0032) != 1)
    {
...
btreeGetTreeSize(BTree *tree)
{
    ...
    rawFileSeek(fp, block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET, SEEK_SET);
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET;
    if (rawFileRead(fp, __offset, &tree_size, sizeof(uint32_t), 1, LOC_BTREE_0033) != 1)
    {
...
void btreeDebug0(BTree *tree, const word_t location)
{
    ...
    if (rawFileRead(fp, __offset, &order, sizeof(uint8_t), 1, LOC_BTREE_0034) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_SIZE_OFFSET;
    if (rawFileRead(fp, __offset, &size, sizeof(uint32_t), 1, LOC_BTREE_0035) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_ROOT_OFFSET;
    if (rawFileRead(fp, __offset, &root, sizeof(offset_t), 1, LOC_BTREE_0036) != 1)
    {
    ...
    __offset = block->offset + GDB_BLOCK_HEADER_SIZE + BTREE_LEFT_LEAF_OFFSET;
    if (rawFileRead(fp, __offset, &leftLeaf, sizeof(offset_t), 1, LOC_BTREE_0037) != 1)
    {
    ...
    if (rawFileRead(fp, __offset, &freeBlockCount, sizeof(uint32_t), 1, LOC_BTREE_0038) != 1)
    {
