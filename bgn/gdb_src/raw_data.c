...
uint8_t   rawDataFlush(RawData *raw_data, const RawFile *raw_file)
{
    if(RAWDATA_IS_DIRTY(raw_data))
    {
        ...
        SAFE_FREE(des_buff, LOC_RAW_0024);
...
uint8_t   rawDataFlush0(RawData *raw_data, const RawFile *raw_file)
{
    if(RAWDATA_IS_DIRTY(raw_data))
    {
        ...
        des_buff = (uint8_t *)SAFE_MALLOC(raw_data->cur_size + des_offset, LOC_RAW_0026);
        if(NULL == des_buff)
        {
            cstring_free(fname_cstr);
        ...
        cbytes_umount(&cbytes);
        cstring_free(fname_cstr);
        SAFE_FREE(des_buff, LOC_RAW_0029);
...
uint8_t   rawDataFlush(RawData *raw_data, const RawFile *raw_file)
{
    if(RAWDATA_IS_DIRTY(raw_data))
    {
        ...
        des_buff = (uint8_t *)SAFE_MALLOC(raw_data->cur_size, LOC_RAW_0031);
        if(NULL == des_buff)
        {
            cstring_free(fname_cstr);
            dbg_log(SEC_0132_RAW, 0)(LOGSTDOUT, "error:rawDataFlush: alloc %d bytes failed\n", raw_data->cur_size);
        ...
        cbytes_umount(&cbytes);
        cstring_free(fname_cstr);
        SAFE_FREE(des_buff, LOC_RAW_0034);
