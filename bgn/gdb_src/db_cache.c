...
gdbCacheAddBlockNoLock(GDatabase *db, GdbBlock *block)
{
    ...
    if (db->openBlockCount >= db->openBlockSize)
    {
        ...
        MEM_CHECK(newBlocks = (GdbBlock **)SAFE_MALLOC(newSize * sizeof(GdbBlock *), LOC_DB_0069));
        ...
        SAFE_FREE(db->openBlocks, LOC_DB_0070);
...
gdbCacheAddBlock(GDatabase *db, GdbBlock *block)
{
    ...
    gdbLockFreeBlockList(db, DB_WRITE_LOCK, LOC_DB_0071);
...
gdbCacheGetBlock(GDatabase *db, offset_t offset)
{
    ...
    gdbLockFreeBlockList(db, DB_READ_LOCK, LOC_DB_0075);
    block = gdbCacheGetBlockNoLock(db, offset);
    gdbUnlockFreeBlockList(db, LOC_DB_0076);
    return block;
}
...
{
    gdbLockFreeBlockList(db, DB_READ_LOCK, LOC_DB_0077);
    gdbCachePrintBlockNoLock(log, db);
    gdbUnlockFreeBlockList(db, LOC_DB_0078);
    return;
}
