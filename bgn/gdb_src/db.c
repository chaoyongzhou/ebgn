...
__setupDatabase(GDatabase *db)
{
    ...
    MEM_CHECK(db->openBlocks = (GdbBlock **)SAFE_MALLOC(db->openBlockSize * sizeof(GdbBlock *), LOC_DB_0022));
...
static uint8_t *__genIdxFileName(const uint8_t *root_path, const word_t table_id)
{
    ...
    MEM_CHECK(fidx_name = (uint8_t *)SAFE_MALLOC(len, LOC_DB_0023));
...
static uint8_t *__genDatFileName(const uint8_t *root_path, const word_t table_id)
{
    ...
    MEM_CHECK(fdat_name = (uint8_t *)SAFE_MALLOC(len, LOC_DB_0024));
...
{
    uint8_t *file_name;
    MEM_CHECK(file_name = (uint8_t *)SAFE_MALLOC(strlen((char *)root_path) + 1, LOC_DB_0025));
...
gdbMake(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id, const int idx_fd, const int dat_fd)
{
    ...
    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0026));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0027);
    ...
    SAFE_FREE(fidx_name, LOC_DB_0028);
    SAFE_FREE(fdat_name, LOC_DB_0029);
...
gdbOpen(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id, const int flags)
{
    ...
    if(flags & O_RDWR)
    {
        ...
        SAFE_FREE(fidx_name, LOC_DB_0030);
    }
    ...
    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0031));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0032);
...
gdbCreate(const uint8_t *root_path, const word_t table_id, const word_t cdfs_md_id)
{
    ...
    SAFE_FREE(fidx_name, LOC_DB_0037);
    ...
    if(NULL == datRawFile)
    {
        ...
        SAFE_FREE(fdat_name, LOC_DB_0038);
        rawFileClose(idxRawFile);
        return NULL;
    }
    SAFE_FREE(fdat_name, LOC_DB_0039);

    MEM_CHECK(db = (GDatabase *)SAFE_MALLOC(sizeof(GDatabase), LOC_DB_0040));
    memset(db, 0, sizeof(GDatabase));
    gdbInitlockFreeBlockList(db, LOC_DB_0041);
...
gdbDestroy(GDatabase *db)
{
    ...
    SAFE_FREE(db->openBlocks, LOC_DB_0042);
    SAFE_FREE(db->filename, LOC_DB_0043);
    SAFE_FREE(db, LOC_DB_0044);
...
gdbUnlink(const uint8_t *root_path, const word_t table_id)
{
    ...
    SAFE_FREE(fidx_name, LOC_DB_0045);
    SAFE_FREE(fdat_name, LOC_DB_0046);
...
gdbInsertKeyValue(GDatabase *db, const KeyValue *keyValue, const uint8_t replaceDup)
{
    ...
    if(NULL != db->datRawFile)
    {
        ...
        MEM_CHECK(kv  = kvNewHs(keyValue, LOC_DB_0047));
        kvPutHs(kv, keyValue);
    ...
    if(RAW_FILE_SUCC == result)
    {
        ...
        MEM_CHECK(key = keyNewHs(klen, LOC_DB_0049));
...
gdbDeleteKey(GDatabase *db, const uint8_t *key, int(* keyCompare)(const uint8_t *, const uint8_t *), offset_t *offset)
{
    ...
    BTREE_CRWLOCK_WRLOCK(db->mainTree, LOC_DB_0051);
    (*offset) = btreeDelete(db->mainTree, key);
    BTREE_CRWLOCK_UNLOCK(db->mainTree, LOC_DB_0052);
    if(0 == (*offset))
    {
        return GDB_ERROR;
    }
...
gdbFetchKey(const GDatabase *db, const offset_t offset, uint8_t **key, uint16_t *key_len)
{
    ...
    (*key) = (uint8_t *)SAFE_MALLOC(tlen, LOC_DB_0053);
    if(NULL == (*key))
    {
...
gdbFetchValue(const GDatabase *db, const offset_t offset, uint8_t **val, uint32_t *val_len)
{
    ...
    (*val) = (uint8_t *)SAFE_MALLOC(vlen, LOC_DB_0055);
    if(NULL == (*val))
    {
...
gdbFetchKV(const GDatabase *db, const offset_t offset, uint8_t **kv, uint32_t *kv_len)
{
    ...
    (*kv) = (uint8_t *)SAFE_MALLOC(dlen, LOC_DB_0057);
    if(NULL == (*kv))
    {
...
gdbKeyRegexScanKVOffset(const GDatabase *db, int (*key_regex)(const uint8_t *, pcre *, pcre *, pcre *), pcre *row_re, pcre *colf_re, pcre *colq_re, CMAP *kvoffset_map)
{
         ...
         offset = btreeGetNextOffset(trav))
    {
        ...
        if(0 != key_regex(key, row_re, colf_re, colq_re))
        {
            ...
            offset_word = offset;
            cmap_add(kvoffset_map, (void *)offset_word, (void *)kv_bytes, LOC_DB_0062);
            continue;
        }

        SAFE_FREE(kv, LOC_DB_0063);
    }
...
gdbKVRegexScanKVOffset(const GDatabase *db, int (*kv_regex)(const uint8_t *, pcre *, pcre *, pcre *, pcre *), pcre *row_re, pcre *colf_re, pcre *colq_re, pcre *val_re, CMAP *kvoffset_map)
{
         ...
         offset = btreeGetNextOffset(trav))
    {
        ...
        if(0 != kv_regex(key, row_re, colf_re, colq_re, val_re))
        {
            ...
            offset_word = offset;
            cmap_add(kvoffset_map, (void *)offset_word, (void *)kv_bytes, LOC_DB_0067);
            continue;
        }

        SAFE_FREE(kv, LOC_DB_0068);
    }
